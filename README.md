...
3. Your script can `export` an `options` object that k6 reads and uses to set any options you want; for example, setting VUs would look like this:
    ...
    export let options = ***REMOVED***
        vus: 30,
    ***REMOVED***;
    export default function() ***REMOVED*** /* ... do whatever ... */ ***REMOVED***
    ```
    ...
    ```js
    if (__ENV.script_scenario == "staging") ***REMOVED***
        export let options = ***REMOVED*** /* first set of options */ ***REMOVED***;
    ***REMOVED*** else ***REMOVED***
        export let options = ***REMOVED*** /* second set of options */ ***REMOVED***;
    ***REMOVED***
    ```
...
4. A global JSON config. By default, k6 looks for it in the config home folder of the current user (OS-dependent, for Linux/BSDs k6 will look for `config.json` inside of `$***REMOVED***HOME***REMOVED***/.config/loadimpact/k6`), though that can be modified with the `--config`/`-c` CLI flag.
It uses the same option keys as the exported `options` from the script file, so we can set the VUs by having `config.json` contain `***REMOVED*** "vus": 1 ***REMOVED***`. Although it rarely makes sense to set the number of VUs there, the global config file is much more useful for storing things like login credentials for the different [outputs](#outputs), as used by the `k6 login` subcommand...
...
For more complex cases, you can specify execution stages. They are a combination of `duration,target-VUs` pairs. These pairs instruct k6 to linearly ramp up, ramp down, or stay at the number of VUs specified for the period specified. Execution stages can be set via the `stages` script/JSON option as an array of `***REMOVED*** duration: ..., target: ... ***REMOVED***` pairs, or with the `--stage`/`-s` CLI flags and the `K6_STAGES` environment variable via the `duration:target,duration:target...` syntax.
...
export let options = ***REMOVED***
    vus: 5,
    stages: [
        ***REMOVED*** duration: "3m", target: 10 ***REMOVED***,
        ***REMOVED*** duration: "5m", target: 10 ***REMOVED***,
        ***REMOVED*** duration: "10m", target: 35 ***REMOVED***,
        ***REMOVED*** duration: "1m30s", target: 0 ***REMOVED***,
    ]
***REMOVED***;
```
