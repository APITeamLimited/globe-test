...
func run(gs *lib.GlobalState, orchestratorId string, orchestratorClient, scopesClient *redis.Client, workerClients map[string]*redis.Client, job map[string]string, storeMongoDB *mongo.Database) ***REMOVED***
	// Get the scope
	...
	***REMOVED***
	...
	for msg := range workerSubscription.Channel() ***REMOVED***
		workerMessage := WorkerMessage***REMOVED******REMOVED***
