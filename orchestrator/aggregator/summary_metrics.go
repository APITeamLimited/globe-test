...
func (sb *summaryBank) CalculateAndDispatchSummaryMetrics() error {
	...
	metricTypes := make([]metrics.MetricType, 0)
	...
	for location, zoneSummaryMetrics := range sb.collectedSummaryMetrics {
		...
		for i, metricKey := range metricKeys {
			...
			if metricTypes[i] == metrics.Counter {
				// Calculate the total for this metric
				outputSummaryMetrics[location][metricKey] = determineSummaryCounter(matchingKeyMetrics, metricKey, metricContains[i], metrics.Counter)
			} else if metricTypes[i] == metrics.Gauge {
				// Gauges are summed
				outputSummaryMetrics[location][metricKey] = determineSummaryCounter(matchingKeyMetrics, metricKey, metricContains[i], metrics.Gauge)
			} else if metricTypes[i] == metrics.Rate {
				// Rates are summed
				outputSummaryMetrics[location][metricKey] = determineSummaryCounter(matchingKeyMetrics, metricKey, metricContains[i], metrics.Counter)
			} else if metricTypes[i] == metrics.Trend {
				// Calculate the trend for this metric
				outputSummaryMetrics[location][metricKey] = determineSummaryTrend(matchingKeyMetrics, metricKey, metricContains[i], metrics.Trend)
			} else {
...
func determineSummaryCounter(matchingKeyMetrics []map[string]wrappedSummaryMetric, metricName string,
	metricContains string, metricType metrics.MetricType) summaryMetric {
