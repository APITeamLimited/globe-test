...
func TestNewJSRunnerWithCustomModule(t *testing.T) ***REMOVED***
	...
	runner, err := js.New(
		...
		***REMOVED***,
		&loader.SourceData***REMOVED***
			...
			Data: []byte(script),
		***REMOVED***,
		map[string]afero.Fs***REMOVED***"file": afero.NewMemMapFs(), "https": afero.NewMemMapFs()***REMOVED***, redis.NewClient(&redis.Options***REMOVED***
			Addr:     "localhost:6379",
	...
	vu, err := runner.NewVU(1, 1, make(chan metrics.SampleContainer, 100), redis.NewClient(&redis.Options***REMOVED***
		Addr:     "localhost:6379",
		Password: "", // no password set
		DB:       0,  // use default DB
	***REMOVED***))
	...
	assert.Equal(t, checkModule.vuCtxCalled, 2)
	vuFromArc, err := runnerFromArc.NewVU(2, 2, make(chan metrics.SampleContainer, 100), redis.NewClient(&redis.Options***REMOVED***
		Addr:     "localhost:6379",
		Password: "", // no password set
		DB:       0,  // use default DB
	***REMOVED***))
