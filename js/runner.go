...
func (r *Runner) newVU() (*VU, error) ***REMOVED***
	...
	***REMOVED***
	...
	hosts := map[string]*ring.Ring***REMOVED******REMOVED***
	for host, ips := range r.Bundle.Options.Hosts ***REMOVED***
		hosts[host] = ring.New(len(ips))

		for i := 0; i < len(ips); i++ ***REMOVED***
...
func (u *VU) RunOnce(ctx context.Context) ([]stats.Sample, error) ***REMOVED***
	...
	*u.Context = ctx
	...
	samples := append(state.Samples,
		stats.Sample***REMOVED***Time: t, Metric: metrics.DataSent, Value: float64(state.BytesWritten), Tags: tags***REMOVED***,
		stats.Sample***REMOVED***Time: t, Metric: metrics.DataReceived, Value: float64(state.BytesRead), Tags: tags***REMOVED***,
