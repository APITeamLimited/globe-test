...
type ExecutorConfig interface ***REMOVED***
	...
	// has been specified.
	//
	// TODO: use interface***REMOVED******REMOVED*** so plain http requests can be specified?
	GetExec() string
...
type InitVUFunc func(context.Context, *logrus.Entry, *WorkerInfo) (InitializedVU, error)
...
func RegisterExecutorConfigType(configType string, constructor ExecutorConfigConstructor) ***REMOVED***
	...
	***REMOVED***
	if _, configTypeExists := executorConfigConstructors[configType]; configTypeExists ***REMOVED***
		panic("executor configs: libWorker.RegisterExecutorConfigType called twice for  " + configType)
	***REMOVED***
...
func (scs ScenarioConfigs) GetFullExecutionRequirements(et *ExecutionTuple) []ExecutionStep ***REMOVED***
	...
	***REMOVED***
	...
	sort.SliceStable(trackedSteps, func(a, b int) bool ***REMOVED***
		if trackedSteps[a].TimeOffset == trackedSteps[b].TimeOffset ***REMOVED***
