...
var _ libWorker.ExecutorConfig = &ExternallyControlledConfig***REMOVED******REMOVED***

// GetDescription returns a human-readable description of the executor options
func (mec ExternallyControlledConfig) GetDescription(_ *libWorker.ExecutionTuple) string ***REMOVED***
	duration := "infinite"
...
// isn't supported.
func (mec ExternallyControlledConfig) GetExecutionRequirements(et *libWorker.ExecutionTuple) []libWorker.ExecutionStep ***REMOVED***
	startVUs := libWorker.ExecutionStep***REMOVED***
		TimeOffset:      0,
	...
	***REMOVED***
	...
	if maxDuration == 0 ***REMOVED***
		...
		return []libWorker.ExecutionStep***REMOVED***startVUs***REMOVED***
	***REMOVED***
	return []libWorker.ExecutionStep***REMOVED***startVUs, ***REMOVED***
...
func (mec ExternallyControlledConfig) NewExecutor(es *libWorker.ExecutionState, logger *logrus.Entry) (libWorker.Executor, error) ***REMOVED***
...
func (mec ExternallyControlledConfig) HasWork(_ *libWorker.ExecutionTuple) bool ***REMOVED***
...
// libWorker.PausableExecutor and the libWorker.LiveUpdatableExecutor interfaces.
...
var (
	_ libWorker.Executor              = &ExternallyControlled***REMOVED******REMOVED***
	_ libWorker.PausableExecutor      = &ExternallyControlled***REMOVED******REMOVED***
	_ libWorker.LiveUpdatableExecutor = &ExternallyControlled***REMOVED******REMOVED***
)
...
func (mex *ExternallyControlled) GetConfig() libWorker.ExecutorConfig ***REMOVED***
...
func (mex *ExternallyControlled) UpdateConfig(ctx context.Context, newConf interface***REMOVED******REMOVED***) error ***REMOVED***
	...
	***REMOVED***
	if errs := newConfigParams.Validate(); len(errs) != 0 ***REMOVED***
		return fmt.Errorf("invalid configuration supplied: %s", libWorker.ConcatErrors(errs, ", "))
	***REMOVED***
...
type manualVUHandle struct ***REMOVED***
	*vuHandle
	initVU libWorker.InitializedVU
	wg     *sync.WaitGroup
...
func (rs *externallyControlledRunState) newManualVUHandle(
	initVU libWorker.InitializedVU, logger *logrus.Entry,
	...
	getVU := func() (libWorker.InitializedVU, error) ***REMOVED***
		...
		return initVU, nil
	***REMOVED***
	returnVU := func(_ libWorker.InitializedVU) ***REMOVED***
		state.ModCurrentlyActiveVUsCount(-1)
		atomic.AddInt64(rs.activeVUsCount, -1)
		wg.Done()
	***REMOVED***
...
func (rs *externallyControlledRunState) handleConfigChange(oldCfg, newCfg ExternallyControlledConfigParams, workerInfo *libWorker.WorkerInfo) error ***REMOVED***
	...
	***REMOVED***).Debug("Updating execution configuration...")
	...
	for i := oldMaxVUs; i < newMaxVUs; i++ ***REMOVED***
		...
		default: // do nothing
		***REMOVED***
		initVU, vuInitErr := executionState.InitializeNewVU(rs.ctx, rs.executor.logger, workerInfo)
...
func (mex *ExternallyControlled) Run(parentCtx context.Context, out chan<- workerMetrics.SampleContainer, workerInfo *libWorker.WorkerInfo) (err error) ***REMOVED***
	...
	***REMOVED***
	...
	ss := &libWorker.ScenarioState***REMOVED***
		Name:      mex.config.Name,
		Executor:  mex.config.Type,
		StartTime: time.Now(),
	***REMOVED***
	ctx = libWorker.WithScenarioState(ctx, ss)
	...
	***REMOVED***()
	...
	err = runState.handleConfigChange( // Start by setting MaxVUs to the starting MaxVUs
		ExternallyControlledConfigParams***REMOVED***MaxVUs: mex.config.MaxVUs***REMOVED***, currentControlConfig, workerInfo,
	)
	if err != nil ***REMOVED***
		return err
	***REMOVED***
	defer func() ***REMOVED*** // Make sure we release the VUs at the end
		err = runState.handleConfigChange(currentControlConfig, ExternallyControlledConfigParams***REMOVED******REMOVED***, workerInfo)
	***REMOVED***()
	...
	for ***REMOVED***
		...
		case updateConfigEvent := <-mex.newControlConfigs:
			err := runState.handleConfigChange(currentControlConfig, updateConfigEvent.newConfig, workerInfo)
			if err != nil ***REMOVED***
