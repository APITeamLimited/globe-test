...
var _ libWorker.ExecutorConfig = &ExternallyControlledConfig{}

// GetDescription returns a human-readable description of the executor options
func (mec ExternallyControlledConfig) GetDescription(_ *libWorker.ExecutionTuple) string {
	duration := "infinite"
...
// isn't supported.
func (mec ExternallyControlledConfig) GetExecutionRequirements(et *libWorker.ExecutionTuple) []libWorker.ExecutionStep {
	startVUs := libWorker.ExecutionStep{
		TimeOffset:      0,
	...
	if maxDuration == 0 {
		...
		return []libWorker.ExecutionStep{startVUs}
	}
	return []libWorker.ExecutionStep{startVUs, {
...
func (mec ExternallyControlledConfig) NewExecutor(es *libWorker.ExecutionState, logger *logrus.Entry) (libWorker.Executor, error) {
...
func (mec ExternallyControlledConfig) HasWork(_ *libWorker.ExecutionTuple) bool {
...
var (
	_ libWorker.Executor              = &ExternallyControlled{}
	_ libWorker.PausableExecutor      = &ExternallyControlled{}
	_ libWorker.LiveUpdatableExecutor = &ExternallyControlled{}
)
...
func (mex *ExternallyControlled) GetConfig() libWorker.ExecutorConfig {
...
func (rs *externallyControlledRunState) newManualVUHandle(
	...
	returnVU := func(_ libWorker.InitializedVU) {
		state.ModCurrentlyActiveVUsCount(-1)
		atomic.AddInt64(rs.activeVUsCount, -1)
		wg.Done()
	}
...
func (rs *externallyControlledRunState) handleConfigChange(oldCfg, newCfg ExternallyControlledConfigParams, workerInfo *libWorker.WorkerInfo) error {
...
func (mex *ExternallyControlled) Run(parentCtx context.Context, out chan<- workerMetrics.SampleContainer, workerInfo *libWorker.WorkerInfo) (err error) {
	mex.configLock.RLock()
	...
	*runState.maxVUs = startMaxVUs
	...
	err = runState.handleConfigChange( // Start by setting MaxVUs to the starting MaxVUs
		ExternallyControlledConfigParams{MaxVUs: mex.config.MaxVUs}, currentControlConfig, workerInfo,
	)
	if err != nil {
		return err
	}
	defer func() { // Make sure we release the VUs at the end
		err = runState.handleConfigChange(currentControlConfig, ExternallyControlledConfigParams{}, workerInfo)
	}()
