...
func Convert(h HAR, options libWorker.Options, minSleep, maxSleep uint, enableChecks bool, returnOnFailedCheck bool, batchTime uint, nobatch bool, correlate bool, only, skip []string) (result string, convertErr error) ***REMOVED***
	var b bytes.Buffer
	w := bufio.NewWriter(&b)
