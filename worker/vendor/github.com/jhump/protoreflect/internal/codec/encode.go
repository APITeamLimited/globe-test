...
func (cb *Buffer) EncodeRawBytes(b []byte) error ***REMOVED***
	if err := cb.EncodeVarint(uint64(len(b))); err != nil ***REMOVED***
		return err
	***REMOVED***
...
func (cb *Buffer) EncodeDelimitedMessage(pm proto.Message) error ***REMOVED***
	...
	***REMOVED***
	...
	if cap(bytes) > cap(cb.tmp) ***REMOVED***
		cb.tmp = bytes[:0]
	***REMOVED***
	return cb.EncodeRawBytes(bytes)
***REMOVED***

func marshalMessage(b []byte, pm proto.Message, deterministic bool) ([]byte, error) ***REMOVED***
	...
	if deterministic ***REMOVED***
		...
		***REMOVED***
		...
		if err := buf.Marshal(pm); err != nil ***REMOVED***
			return nil, err
		***REMOVED***
		bytes := buf.Bytes()
		if len(b) == 0 ***REMOVED***
