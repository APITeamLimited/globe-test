package ftoa

import (
	"math"
	"strconv"

	"github.com/dop251/goja/ftoa/internal/fast"
)

type FToStrMode int

const (
	// Either fixed or exponential format; round-trip
	ModeStandard FToStrMode = iota
	// Always exponential format; round-trip
	ModeStandardExponential
	// Round to <precision> digits after the decimal point; exponential if number is large
	ModeFixed
	// Always exponential format; <precision> significant digits
	ModeExponential
	// Either fixed or exponential format; <precision> significant digits
	ModePrecision
)

func insert(b []byte, p int, c byte) []byte ***REMOVED***
	b = append(b, 0)
	copy(b[p+1:], b[p:])
	b[p] = c
	return b
***REMOVED***

func expand(b []byte, delta int) []byte ***REMOVED***
	newLen := len(b) + delta
	if newLen <= cap(b) ***REMOVED***
		return b[:newLen]
	***REMOVED***
	b1 := make([]byte, newLen)
	copy(b1, b)
	return b1
***REMOVED***

func FToStr(d float64, mode FToStrMode, precision int, buffer []byte) []byte ***REMOVED***
	if math.IsNaN(d) ***REMOVED***
		buffer = append(buffer, "NaN"...)
		return buffer
	***REMOVED***
	if math.IsInf(d, 0) ***REMOVED***
		if math.Signbit(d) ***REMOVED***
			buffer = append(buffer, '-')
		***REMOVED***
		buffer = append(buffer, "Infinity"...)
		return buffer
	***REMOVED***

	if mode == ModeFixed && (d >= 1e21 || d <= -1e21) ***REMOVED***
		mode = ModeStandard
	***REMOVED***

	var decPt int
	var ok bool
	startPos := len(buffer)

	if d != 0 ***REMOVED*** // also matches -0
		if d < 0 ***REMOVED***
			buffer = append(buffer, '-')
			d = -d
			startPos++
		***REMOVED***
		switch mode ***REMOVED***
		case ModeStandard, ModeStandardExponential:
			buffer, decPt, ok = fast.Dtoa(d, fast.ModeShortest, 0, buffer)
		case ModeExponential, ModePrecision:
			buffer, decPt, ok = fast.Dtoa(d, fast.ModePrecision, precision, buffer)
		***REMOVED***
	***REMOVED*** else ***REMOVED***
		buffer = append(buffer, '0')
		decPt, ok = 1, true
	***REMOVED***
	if !ok ***REMOVED***
		buffer, decPt = ftoa(d, dtoaModes[mode], mode >= ModeFixed, precision, buffer)
	***REMOVED***
	exponentialNotation := false
	minNDigits := 0 /* Minimum number of significand digits required by mode and precision */
	nDigits := len(buffer) - startPos

	switch mode ***REMOVED***
	case ModeStandard:
		if decPt < -5 || decPt > 21 ***REMOVED***
			exponentialNotation = true
		***REMOVED*** else ***REMOVED***
			minNDigits = decPt
		***REMOVED***
	case ModeFixed:
		if precision >= 0 ***REMOVED***
			minNDigits = decPt + precision
		***REMOVED*** else ***REMOVED***
			minNDigits = decPt
		***REMOVED***
	case ModeExponential:
		//                    JS_ASSERT(precision > 0);
		minNDigits = precision
		fallthrough
	case ModeStandardExponential:
		exponentialNotation = true
	case ModePrecision:
		//                    JS_ASSERT(precision > 0);
		minNDigits = precision
		if decPt < -5 || decPt > precision ***REMOVED***
			exponentialNotation = true
		***REMOVED***
	***REMOVED***

	for nDigits < minNDigits ***REMOVED***
		buffer = append(buffer, '0')
		nDigits++
	***REMOVED***

	if exponentialNotation ***REMOVED***
		/* Insert a decimal point if more than one significand digit */
		if nDigits != 1 ***REMOVED***
			buffer = insert(buffer, startPos+1, '.')
		***REMOVED***
		buffer = append(buffer, 'e')
		if decPt-1 >= 0 ***REMOVED***
			buffer = append(buffer, '+')
		***REMOVED***
		buffer = strconv.AppendInt(buffer, int64(decPt-1), 10)
	***REMOVED*** else if decPt != nDigits ***REMOVED***
		/* Some kind of a fraction in fixed notation */
		//                JS_ASSERT(decPt <= nDigits);
		if decPt > 0 ***REMOVED***
			/* dd...dd . dd...dd */
			buffer = insert(buffer, startPos+decPt, '.')
		***REMOVED*** else ***REMOVED***
			/* 0 . 00...00dd...dd */
			buffer = expand(buffer, 2-decPt)
			copy(buffer[startPos+2-decPt:], buffer[startPos:])
			buffer[startPos] = '0'
			buffer[startPos+1] = '.'
			for i := startPos + 2; i < startPos+2-decPt; i++ ***REMOVED***
				buffer[i] = '0'
			***REMOVED***
		***REMOVED***
	***REMOVED***

	return buffer
***REMOVED***
