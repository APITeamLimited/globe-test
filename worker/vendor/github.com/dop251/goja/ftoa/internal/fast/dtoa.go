...
func roundWeed(buffer []byte, distance_too_high_w, unsafe_interval, rest, ten_kappa, unit uint64) bool ***REMOVED***
	...
	//              (buffer***REMOVED***-1***REMOVED*** < w_high) && w_high - buffer***REMOVED***-1***REMOVED*** > buffer - w_high
	...
	for rest < small_distance && // Negated condition 1
		...
		(rest+ten_kappa < small_distance || // buffer***REMOVED***-1***REMOVED*** > w_high
			small_distance-rest >= rest+ten_kappa-small_distance) ***REMOVED***
		buffer[len(buffer)-1]--
		rest += ten_kappa
	***REMOVED***
	...
	if rest < big_distance && unsafe_interval-rest >= ten_kappa &&
		(rest+ten_kappa < big_distance ||
			big_distance-rest > rest+ten_kappa-big_distance) ***REMOVED***
		return false
	***REMOVED***
...
func roundWeedCounted(buffer []byte, rest, ten_kappa, unit uint64, kappa *int) bool ***REMOVED***
	...
	if unit >= ten_kappa ***REMOVED***
		return false
	***REMOVED***
	// Even if unit is just half the size of 10^kappa we are already completely
	// lost. (And after the previous test we know that the expression will not
	// over/underflow.)
	if ten_kappa-unit <= unit ***REMOVED***
		return false
	***REMOVED***
	// If 2 * (rest + unit) <= 10^kappa we can safely round down.
	if (ten_kappa-rest > rest) && (ten_kappa-2*rest >= 2*unit) ***REMOVED***
		return true
	***REMOVED***

	// If 2 * (rest - unit) >= 10^kappa, then we can safely round up.
	if (rest > unit) && (ten_kappa-(rest-unit) <= (rest - unit)) ***REMOVED***
		// Increment the last digit recursively until we find a non '9' digit.
		buffer[len(buffer)-1]++
		for i := len(buffer) - 1; i > 0; i-- ***REMOVED***
			if buffer[i] != '0'+10 ***REMOVED***
				break
			***REMOVED***
			buffer[i] = '0'
			buffer[i-1]++
...
func biggestPowerTen(number uint32, number_bits int) (power uint32, exponent int) ***REMOVED***
	switch number_bits ***REMOVED***
	case 32, 31, 30:
		if kTen9 <= number ***REMOVED***
			power = kTen9
			exponent = 9
			break
		***REMOVED***
...
// imprecision.
func digitGen(low, w, high diyfp, buffer []byte) (kappa int, buf []byte, res bool) ***REMOVED***
	...
	unit := uint64(1)
	too_low := diyfp***REMOVED***f: low.f - unit, e: low.e***REMOVED***
	too_high := diyfp***REMOVED***f: high.f + unit, e: high.e***REMOVED***
	...
	one := diyfp***REMOVED***f: 1 << -w.e, e: w.e***REMOVED***
	...
	for kappa > 0 ***REMOVED***
		...
		if rest < unsafe_interval.f ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		kappa--
		if fractionals < unsafe_interval.f ***REMOVED***
...
func digitGenCounted(w diyfp, requested_digits int, buffer []byte) (kappa int, buf []byte, res bool) ***REMOVED***
	_DCHECK(kMinimalTargetExponent <= w.e && w.e <= kMaximalTargetExponent)
	...
	***REMOVED***
	...
	_DCHECK(fractionals < one.f)
	_DCHECK(0xFFFFFFFFFFFFFFFF/10 >= one.f)
	for requested_digits > 0 && fractionals > w_error ***REMOVED***
		fractionals *= 10
		w_error *= 10
		// Integer division by one.
		digit := byte(fractionals >> -one.e)
...
// computed.
func grisu3(f float64, buffer []byte) (digits []byte, decimal_exponent int, result bool) ***REMOVED***
	v := double(f)
...
func grisu3Counted(v float64, requested_digits int, buffer []byte) (digits []byte, decimal_exponent int, result bool) ***REMOVED***
...
func Dtoa(v float64, mode Mode, requested_digits int, buffer []byte) (digits []byte, decimal_point int, result bool) ***REMOVED***
	defer func() ***REMOVED***
		if x := recover(); x != nil ***REMOVED***
			if x == dcheckFailure ***REMOVED***
