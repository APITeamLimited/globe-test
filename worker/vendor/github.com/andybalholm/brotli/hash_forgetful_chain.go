...
func (*hashForgetfulChain) HashTypeLength() uint ***REMOVED***
	return 4
***REMOVED***

func (*hashForgetfulChain) StoreLookahead() uint ***REMOVED***
	return 4
***REMOVED***

/* HashBytes is the function that chooses the bucket to place the address in.*/
func (h *hashForgetfulChain) HashBytes(data []byte) uint ***REMOVED***
...
type hashForgetfulChain struct ***REMOVED***
	hasherCommon
...
func (h *hashForgetfulChain) Initialize(params *encoderParams) ***REMOVED***
	var q uint
	if params.quality > 6 ***REMOVED***
		q = 7
	...
	***REMOVED***
	...
	for i := range h.banks ***REMOVED***
		h.banks[i] = make([]slot, bankSize)
	***REMOVED***
	h.free_slot_idx = make([]uint16, h.numBanks)
***REMOVED***

func (h *hashForgetfulChain) Prepare(one_shot bool, input_size uint, data []byte) ***REMOVED***
	var partial_prepare_threshold uint = (1 << h.bucketBits) >> 6
	/* Partial preparation is 100 times slower (per socket). */
	if one_shot && input_size <= partial_prepare_threshold ***REMOVED***
		var i uint
	...
	***REMOVED*** else ***REMOVED***
		...
		for i := range h.addr ***REMOVED***
			h.addr[i] = 0xCCCCCCCC
		***REMOVED***

		for i := range h.head ***REMOVED***
			h.head[i] = 0
		***REMOVED***
	***REMOVED***

	h.tiny_hash = [65536]byte***REMOVED******REMOVED***
	for i := range h.free_slot_idx ***REMOVED***
		h.free_slot_idx[i] = 0
	***REMOVED***
***REMOVED***
...
func (h *hashForgetfulChain) Store(data []byte, mask uint, ix uint) ***REMOVED***
...
func (h *hashForgetfulChain) StoreRange(data []byte, mask uint, ix_start uint, ix_end uint) ***REMOVED***
	var i uint
	for i = ix_start; i < ix_end; i++ ***REMOVED***
		h.Store(data, mask, i)
	***REMOVED***
***REMOVED***

func (h *hashForgetfulChain) StitchToPreviousBlock(num_bytes uint, position uint, ringbuffer []byte, ring_buffer_mask uint) ***REMOVED***
	if num_bytes >= h.HashTypeLength()-1 && position >= 3 ***REMOVED***
...
func (h *hashForgetfulChain) PrepareDistanceCache(distance_cache []int) ***REMOVED***
...
func (h *hashForgetfulChain) FindLongestMatch(dictionary *encoderDictionary, data []byte, ring_buffer_mask uint, distance_cache []int, cur_ix uint, max_length uint, max_backward uint, gap uint, max_distance uint, out *hasherSearchResult) ***REMOVED***
	...
	for i := 0; i < h.numLastDistancesToCheck; i++ ***REMOVED***
		var backward uint = uint(distance_cache[i])
		var prev_ix uint = (cur_ix - backward)

		/* For distance code 0 we want to consider 2-byte matches. */
		if i > 0 && h.tiny_hash[uint16(prev_ix)] != tiny_hash ***REMOVED***
			continue
		***REMOVED***
		if prev_ix >= cur_ix || backward > max_backward ***REMOVED***
			continue
		***REMOVED***
	...
	***REMOVED***
		...
		for ***REMOVED***
			...
			***REMOVED***
			...
			if cur_ix_masked+best_len > ring_buffer_mask || prev_ix+best_len > ring_buffer_mask || data[cur_ix_masked+best_len] != data[prev_ix+best_len] ***REMOVED***
				continue
			***REMOVED***
			***REMOVED***
				var len uint = findMatchLengthWithLimit(data[prev_ix:], data[cur_ix_masked:], max_length)
	...
	***REMOVED***
	...
	if out.score == min_score ***REMOVED***
