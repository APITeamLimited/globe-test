...
type ringBuffer struct ***REMOVED***
	size_       uint32
	mask_       uint32
	tail_size_  uint32
	total_size_ uint32
...
func ringBufferInit(rb *ringBuffer) ***REMOVED***
	rb.pos_ = 0
***REMOVED***

func ringBufferSetup(params *encoderParams, rb *ringBuffer) ***REMOVED***
...
func ringBufferInitBuffer(buflen uint32, rb *ringBuffer) ***REMOVED***
	var new_data []byte
	var i uint
	size := 2 + int(buflen) + int(kSlackForEightByteHashingEverywhere)
	if cap(rb.data_) < size ***REMOVED***
		new_data = make([]byte, size)
	***REMOVED*** else ***REMOVED***
		new_data = rb.data_[:size]
	***REMOVED***
	if rb.data_ != nil ***REMOVED***
	...
	***REMOVED***
	...
	for i = 0; i < kSlackForEightByteHashingEverywhere; i++ ***REMOVED***
		rb.buffer_[rb.cur_size_+uint32(i)] = 0
	***REMOVED***
***REMOVED***

func ringBufferWriteTail(bytes []byte, n uint, rb *ringBuffer) ***REMOVED***
	var masked_pos uint = uint(rb.pos_ & rb.mask_)
	if uint32(masked_pos) < rb.tail_size_ ***REMOVED***
...
func ringBufferWrite(bytes []byte, n uint, rb *ringBuffer) ***REMOVED***
	if rb.pos_ == 0 && uint32(n) < rb.tail_size_ ***REMOVED***
	...
	***REMOVED***
	...
	if rb.cur_size_ < rb.total_size_ ***REMOVED***
	...
	***REMOVED***
		...
		if uint32(masked_pos+n) <= rb.size_ ***REMOVED***
			/* A single write fits. */
