...
func initialEntropyCodesDistance(data []uint16, length uint, stride uint, num_histograms uint, histograms []histogramDistance) ***REMOVED***
	var seed uint32 = 7
...
func randomSampleDistance(seed *uint32, data []uint16, length uint, stride uint, sample *histogramDistance) ***REMOVED***
	var pos uint = 0
...
func refineEntropyCodesDistance(data []uint16, length uint, stride uint, num_histograms uint, histograms []histogramDistance) ***REMOVED***
...
func findBlocksDistance(data []uint16, length uint, block_switch_bitcost float64, num_histograms uint, histograms []histogramDistance, insert_cost []float64, cost []float64, switch_signal []byte, block_id []byte) uint ***REMOVED***
...
func remapBlockIdsDistance(block_ids []byte, length uint, new_id []uint16, num_histograms uint) uint ***REMOVED***
	...
	***REMOVED***
	...
	for i = 0; i < length; i++ ***REMOVED***
		...
		if new_id[block_ids[i]] == remapBlockIdsDistance_kInvalidId ***REMOVED***
...
func buildBlockHistogramsDistance(data []uint16, length uint, block_ids []byte, num_histograms uint, histograms []histogramDistance) ***REMOVED***
	var i uint
	clearHistogramsDistance(histograms, num_histograms)
	for i = 0; i < length; i++ ***REMOVED***
...
func clusterBlocksDistance(data []uint16, length uint, num_blocks uint, block_ids []byte, split *blockSplit) ***REMOVED***
	...
	***REMOVED***
		...
		for i = 0; i < num_blocks; i++ ***REMOVED***
			...
			***REMOVED***
			...
			if new_index[best_out] == clusterBlocksDistance_kInvalidIndex ***REMOVED***
...
func splitByteVectorDistance(data []uint16, length uint, literals_per_histogram uint, max_histograms uint, sampling_stride_length uint, block_switch_cost float64, params *encoderParams, split *blockSplit) ***REMOVED***
	var data_size uint = histogramDataSizeDistance()
