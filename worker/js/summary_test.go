...
func TestTextSummaryWithSubMetrics(t *testing.T) {
	...
	require.NoError(t, err)
	parentMetric.Sink.Add(workerMetrics.Sample{Value: 11})

	parentMetricPost, err := registry.NewMetric("my_parent_post", workerMetrics.Counter)
	require.NoError(t, err)
	parentMetricPost.Sink.Add(workerMetrics.Sample{Value: 22})

	subMetric, err := parentMetric.AddSubmetric("sub:1")
	require.NoError(t, err)
	subMetric.Metric.Sink.Add(workerMetrics.Sample{Value: 1})

	subMetricPost, err := parentMetricPost.AddSubmetric("sub:2")
	require.NoError(t, err)
	subMetricPost.Metric.Sink.Add(workerMetrics.Sample{Value: 2})

	metrics := map[string]*workerMetrics.Metric{
...
func createTestMetrics(t *testing.T) (map[string]*workerMetrics.Metric, *libWorker.Group) {
	registry := workerMetrics.NewRegistry()
	testMetrics := make(map[string]*workerMetrics.Metric)

	gaugeMetric, err := registry.NewMetric("vus", workerMetrics.Gauge)
	require.NoError(t, err)
	gaugeMetric.Sink.Add(workerMetrics.Sample{Value: 1})

	countMetric, err := registry.NewMetric("http_reqs", workerMetrics.Counter)
	require.NoError(t, err)
	countMetric.Tainted = null.BoolFrom(true)
	countMetric.Thresholds = workerMetrics.Thresholds{Thresholds: []*workerMetrics.Threshold{{Source: "rate<100", LastFailed: true}}}

	checksMetric, err := registry.NewMetric("checks", workerMetrics.Rate)
	require.NoError(t, err)
	checksMetric.Tainted = null.BoolFrom(false)
	checksMetric.Thresholds = workerMetrics.Thresholds{Thresholds: []*workerMetrics.Threshold{{Source: "rate>70", LastFailed: false}}}
	sink := &workerMetrics.TrendSink{}

	samples := []float64{10.0, 15.0, 20.0}
	for _, s := range samples {
		sink.Add(workerMetrics.Sample{Value: s})
		countMetric.Sink.Add(workerMetrics.Sample{Value: 1})
	}

	testMetrics["vus"] = gaugeMetric
	testMetrics["http_reqs"] = countMetric
	testMetrics["checks"] = checksMetric
	testMetrics["my_trend"] = &workerMetrics.Metric{
		Name:     "my_trend",
		Type:     workerMetrics.Trend,
		Contains: workerMetrics.Time,
		Sink:     sink,
		Tainted:  null.BoolFrom(true),
		Thresholds: workerMetrics.Thresholds{
	...
	for i := 0; i < int(check1.Passes+check2.Passes+check3.Passes); i++ {
		checksMetric.Sink.Add(workerMetrics.Sample{Value: 1})
	}
	for i := 0; i < int(check1.Fails+check2.Fails+check3.Fails); i++ {
		checksMetric.Sink.Add(workerMetrics.Sample{Value: 0})
	}
