...
// transition.
func EnableUintSupport() ***REMOVED***
	enableUint64Support = true
***REMOVED***

// Point defines the values that will be written to the database.
type Point interface ***REMOVED***
...
type FieldIterator interface ***REMOVED***
...
func (a Points) Len() int ***REMOVED*** return len(a) ***REMOVED***

// Less implements sort.Interface.
func (a Points) Less(i, j int) bool ***REMOVED*** return a[i].Time().Before(a[j].Time()) ***REMOVED***

// Swap implements sort.Interface.
func (a Points) Swap(i, j int) ***REMOVED*** a[i], a[j] = a[j], a[i] ***REMOVED***
...
type point struct ***REMOVED***
	...
	cachedFields map[string]interface***REMOVED******REMOVED***
...
// This can have the unintended effect preventing buf from being garbage collected.
func ParsePointsWithPrecision(buf []byte, defaultTime time.Time, precision string) ([]Point, error) ***REMOVED***
	points := make([]Point, 0, bytes.Count(buf, []byte***REMOVED***'\n'***REMOVED***)+1)
	var (
		pos    int
		block  []byte
		failed []string
	)
	for pos < len(buf) ***REMOVED***
		pos, block = scanLine(buf, pos)
		pos++

		if len(block) == 0 ***REMOVED***
			continue
		***REMOVED***
...
func parsePoint(buf []byte, defaultTime time.Time, precision string) (Point, error) ***REMOVED***
	...
	***REMOVED***
	...
	if len(key) > MaxKeyLength ***REMOVED***
...
func GetPrecisionMultiplier(precision string) int64 ***REMOVED***
	d := time.Nanosecond
	switch precision ***REMOVED***
	case "u":
		d = time.Microsecond
	case "ms":
		d = time.Millisecond
...
func scanKey(buf []byte, i int) (int, []byte, error) ***REMOVED***
	...
	***REMOVED***
	...
	for j := 0; j < commas-1; j++ ***REMOVED***
		...
		if cmp := bytes.Compare(left, right); cmp > 0 ***REMOVED***
			sorted = false
			break
	...
	if !sorted && commas > 0 ***REMOVED***
		...
		***REMOVED***
		...
		for j := 0; j < commas-1; j++ ***REMOVED***
			...
			// If the tags are not sorted, this pass may not find duplicate tags and we
			// need to do a more exhaustive search later.
			if bytes.Equal(left, right) ***REMOVED***
...
func scanMeasurement(buf []byte, i int) (int, int, error) ***REMOVED***
	// Check first byte of measurement, anything except a comma is fine.
	// It can't be a space, since whitespace is stripped prior to this
	// function call.
	if i >= len(buf) || buf[i] == ',' ***REMOVED***
...
func scanTags(buf []byte, i int, indices []int) (int, int, []int, error) ***REMOVED***
	var (
		err    error
		commas int
		state  = tagKeyState
...
func scanTagsKey(buf []byte, i int) (int, error) ***REMOVED***
	// First character of the key.
	if i >= len(buf) || buf[i] == ' ' || buf[i] == ',' || buf[i] == '=' ***REMOVED***
		// cpu,***REMOVED***'', ' ', ',', '='***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		if i >= len(buf) ||
			((buf[i] == ' ' || buf[i] == ',') && buf[i-1] != '\\') ***REMOVED***
			// cpu,tag***REMOVED***'', ' ', ','***REMOVED***
			return i, fmt.Errorf("missing tag value")
		***REMOVED***

		if buf[i] == '=' && buf[i-1] != '\\' ***REMOVED***
			// cpu,tag=
			return i + 1, nil
		***REMOVED***
	***REMOVED***
...
func scanTagsValue(buf []byte, i int) (int, int, error) ***REMOVED***
	// Tag value cannot be empty.
	if i >= len(buf) || buf[i] == ',' || buf[i] == ' ' ***REMOVED***
		// cpu,tag=***REMOVED***',', ' '***REMOVED***
...
func insertionSort(l, r int, buf []byte, indices []int) ***REMOVED***
	for i := l + 1; i < r; i++ ***REMOVED***
		for j := i; j > l && less(buf, indices, j, j-1); j-- ***REMOVED***
			indices[j], indices[j-1] = indices[j-1], indices[j]
		***REMOVED***
	***REMOVED***
***REMOVED***

func less(buf []byte, indices []int, i, j int) bool ***REMOVED***
...
func scanFields(buf []byte, i int) (int, []byte, error) ***REMOVED***
	start := skipWhitespace(buf, i)
	i = start
	quoted := false
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		// in the field key
		if buf[i] == '"' && equals > commas ***REMOVED***
			quoted = !quoted
			i++
			continue
		***REMOVED***
		...
		if buf[i] == '=' && !quoted ***REMOVED***
			equals++

			// check for "... =123" but allow "a\ =123"
			if buf[i-1] == ' ' && buf[i-2] != '\\' ***REMOVED***
			...
			***REMOVED***
			...
			// check for "... value="
			if i+1 >= len(buf) ***REMOVED***
				return i, buf[start:i], fmt.Errorf("missing field value")
			***REMOVED***

			// check for "... value=,value2=..."
			if buf[i+1] == ',' || buf[i+1] == ' ' ***REMOVED***
				return i, buf[start:i], fmt.Errorf("missing field value")
			***REMOVED***

			if isNumeric(buf[i+1]) || buf[i+1] == '-' || buf[i+1] == 'N' || buf[i+1] == 'n' ***REMOVED***
				var err error
	...
	***REMOVED***
	...
	if equals == 0 || commas != equals-1 ***REMOVED***
...
func scanTime(buf []byte, i int) (int, []byte, error) ***REMOVED***
	start := skipWhitespace(buf, i)
	i = start
...
func isNumeric(b byte) bool ***REMOVED***
	return (b >= '0' && b <= '9') || b == '.'
***REMOVED***
...
func scanNumber(buf []byte, i int) (int, error) ***REMOVED***
	start := i
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if buf[i] == ',' || buf[i] == ' ' ***REMOVED***
			break
		***REMOVED***

		if buf[i] == 'i' && i > start && !(isInt || isUnsigned) ***REMOVED***
			isInt = true
			i++
			continue
		***REMOVED*** else if buf[i] == 'u' && i > start && !(isInt || isUnsigned) ***REMOVED***
			isUnsigned = true
			i++
			continue
		***REMOVED***
		...
		***REMOVED***
		...
		if i > start && (buf[i] == 'e' || buf[i] == 'E') ***REMOVED***
			scientific = true
			i++
			continue
		***REMOVED***
		...
		if (buf[i] == '+' || buf[i] == '-') && (buf[i-1] == 'e' || buf[i-1] == 'E') ***REMOVED***
			i++
			continue
		***REMOVED***

		// NaN is an unsupported value
		if i+2 < len(buf) && (buf[i] == 'N' || buf[i] == 'n') ***REMOVED***
			return i, ErrInvalidNumber
		***REMOVED***

		if !isNumeric(buf[i]) ***REMOVED***
			return i, ErrInvalidNumber
		***REMOVED***
		i++
	***REMOVED***

	if (isInt || isUnsigned) && (decimal || scientific) ***REMOVED***
		return i, ErrInvalidNumber
	***REMOVED***
	...
	***REMOVED***
	if decimal ***REMOVED***
		numericDigits--
	***REMOVED***
	if buf[start] == '-' ***REMOVED***
		numericDigits--
	***REMOVED***

	if numericDigits == 0 ***REMOVED***
		return i, ErrInvalidNumber
	***REMOVED***
	...
	if isInt ***REMOVED***
		...
		if buf[i-1] != 'i' ***REMOVED***
			return i, ErrInvalidNumber
		***REMOVED***
		// Parse the int to check bounds the number of digits could be larger than the max range
		// We subtract 1 from the index to remove the `i` from our tests
		if len(buf[start:i-1]) >= maxInt64Digits || len(buf[start:i-1]) >= minInt64Digits ***REMOVED***
			if _, err := parseIntBytes(buf[start:i-1], 10, 64); err != nil ***REMOVED***
				return i, fmt.Errorf("unable to parse integer %s: %s", buf[start:i-1], err)
			***REMOVED***
		***REMOVED***
	***REMOVED*** else if isUnsigned ***REMOVED***
		// Return an error if uint64 support has not been enabled.
		if !enableUint64Support ***REMOVED***
			return i, ErrInvalidNumber
		***REMOVED***
		// Make sure the last char is a 'u' for unsigned
		if buf[i-1] != 'u' ***REMOVED***
			return i, ErrInvalidNumber
		***REMOVED***
		...
		***REMOVED***
		...
		if len(buf[start:i-1]) >= maxUint64Digits ***REMOVED***
			if _, err := parseUintBytes(buf[start:i-1], 10, 64); err != nil ***REMOVED***
				return i, fmt.Errorf("unable to parse unsigned %s: %s", buf[start:i-1], err)
			***REMOVED***
		***REMOVED***
	***REMOVED*** else ***REMOVED***
		// Parse the float to check bounds if it's scientific or the number of digits could be larger than the max range
		if scientific || len(buf[start:i]) >= maxFloat64Digits || len(buf[start:i]) >= minFloat64Digits ***REMOVED***
			if _, err := parseFloatBytes(buf[start:i], 10); err != nil ***REMOVED***
...
// is scanned.
func scanBoolean(buf []byte, i int) (int, []byte, error) ***REMOVED***
	start := i

	if i < len(buf) && (buf[i] != 't' && buf[i] != 'f' && buf[i] != 'T' && buf[i] != 'F') ***REMOVED***
	...
	***REMOVED***
	...
	if i-start == 1 ***REMOVED***
		return i, buf[start:i], nil
	***REMOVED***

	// length must be 4 for true or TRUE
	if (buf[start] == 't' || buf[start] == 'T') && i-start != 4 ***REMOVED***
		return i, buf[start:i], fmt.Errorf("invalid boolean")
	***REMOVED***

	// length must be 5 for false or FALSE
	if (buf[start] == 'f' || buf[start] == 'F') && i-start != 5 ***REMOVED***
		return i, buf[start:i], fmt.Errorf("invalid boolean")
	***REMOVED***

	// Otherwise
	valid := false
	switch buf[start] ***REMOVED***
	case 't':
...
func skipWhitespace(buf []byte, i int) int ***REMOVED***
	for i < len(buf) ***REMOVED***
		if buf[i] != ' ' && buf[i] != '\t' && buf[i] != 0 ***REMOVED***
			break
		***REMOVED***
		i++
	***REMOVED***
...
// buf.
func scanLine(buf []byte, i int) (int, []byte) ***REMOVED***
	start := i
	quoted := false
	fields := false
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if fields ***REMOVED***
			if !quoted && buf[i] == '=' ***REMOVED***
				i++
				equals++
				continue
			***REMOVED*** else if !quoted && buf[i] == ',' ***REMOVED***
				i++
				commas++
				continue
			***REMOVED*** else if buf[i] == '"' && equals > commas ***REMOVED***
				i++
				quoted = !quoted
				continue
			***REMOVED***
...
func scanTo(buf []byte, i int, stop byte) (int, []byte) ***REMOVED***
	start := i
	for ***REMOVED***
		// reached the end of buf?
		if i >= len(buf) ***REMOVED***
		...
		***REMOVED***
		...
		if buf[i] == stop && (i == 0 || buf[i-1] != '\\') ***REMOVED***
			break
		***REMOVED***
		i++
	***REMOVED***
...
func scanToSpaceOr(buf []byte, i int, stop byte) (int, []byte) ***REMOVED***
	start := i
	if buf[i] == stop || buf[i] == ' ' ***REMOVED***
		return i, buf[start:i]
	***REMOVED***
...
func scanTagValue(buf []byte, i int) (int, []byte) ***REMOVED***
	start := i
	for ***REMOVED***
		if i >= len(buf) ***REMOVED***
			break
...
func scanFieldValue(buf []byte, i int) (int, []byte) ***REMOVED***
	start := i
	quoted := false
	for i < len(buf) ***REMOVED***
		// Only escape char for a field value is a double-quote and backslash
		if buf[i] == '\\' && i+1 < len(buf) && (buf[i+1] == '"' || buf[i+1] == '\\') ***REMOVED***
			i += 2
			continue
		***REMOVED***
...
func unescapeTag(in []byte) []byte ***REMOVED***
	if bytes.IndexByte(in, '\\') == -1 ***REMOVED***
		return in
	***REMOVED***
...
func EscapeStringField(in string) string ***REMOVED***
...
func unescapeStringField(in string) string ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if in[i] == '\\' && i+1 < len(in) && in[i+1] == '\\' ***REMOVED***
			out = append(out, '\\')
			i += 2
			continue
		***REMOVED***
...
func pointKey(measurement string, tags Tags, fields Fields, t time.Time) ([]byte, error) ***REMOVED***
	if len(fields) == 0 ***REMOVED***
		return nil, ErrPointMustHaveAField
	***REMOVED***

	if !t.IsZero() ***REMOVED***
		if err := CheckTime(t); err != nil ***REMOVED***
			return nil, err
		***REMOVED***
	***REMOVED***
	...
	***REMOVED***
	...
	for field := range fields ***REMOVED***
		sz := seriesKeySize(key, []byte(field))
		if sz > MaxKeyLength ***REMOVED***
...
func seriesKeySize(key, field []byte) int ***REMOVED***
...
func NewPointFromBytes(b []byte) (Point, error) ***REMOVED***
	p := &point***REMOVED******REMOVED***
	if err := p.UnmarshalBinary(b); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	...
	for iter.Next() ***REMOVED***
		if len(iter.FieldKey()) == 0 ***REMOVED***
			continue
		***REMOVED***
		hasField = true
		switch iter.Type() ***REMOVED***
		case Float:
...
func MustNewPoint(name string, tags Tags, fields Fields, time time.Time) Point ***REMOVED***
...
// Key returns the key (measurement joined with tags) of the point.
func (p *point) Key() []byte ***REMOVED***
	return p.key
***REMOVED***

func (p *point) name() []byte ***REMOVED***
...
// SetName updates the measurement name for the point.
func (p *point) SetName(name string) ***REMOVED***
	p.cachedName = ""
...
// SetTime updates the timestamp for the point.
func (p *point) SetTime(t time.Time) ***REMOVED***
	p.time = t
***REMOVED***

// Round will round the timestamp of the point to the given duration.
func (p *point) Round(d time.Duration) ***REMOVED***
	p.time = p.time.Round(d)
***REMOVED***

// Tags returns the tag set for the point.
func (p *point) Tags() Tags ***REMOVED***
...
func (p *point) HasTag(tag []byte) bool ***REMOVED***
	if len(p.key) == 0 ***REMOVED***
		return false
	***REMOVED***

	var exists bool
	walkTags(p.key, func(key, value []byte) bool ***REMOVED***
		if bytes.Equal(tag, key) ***REMOVED***
			exists = true
			return false
		***REMOVED***
		return true
...
func walkTags(buf []byte, fn func(key, value []byte) bool) ***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if hasEscape ***REMOVED***
			if !fn(unescapeTag(key), unescapeTag(value)) ***REMOVED***
				return
			***REMOVED***
		***REMOVED*** else ***REMOVED***
			if !fn(key, value) ***REMOVED***
				return
			***REMOVED***
		***REMOVED***
...
// SetTags replaces the tags for the point.
func (p *point) SetTags(tags Tags) ***REMOVED***
	p.key = MakeKey(p.Name(), tags)
	p.cachedTags = tags
***REMOVED***

// AddTag adds or replaces a tag value for a point.
func (p *point) AddTag(key, value string) ***REMOVED***
	tags := p.Tags()
	tags = append(tags, Tag***REMOVED***Key: []byte(key), Value: []byte(value)***REMOVED***)
	sort.Sort(tags)
	p.cachedTags = tags
	p.key = MakeKey(p.Name(), tags)
***REMOVED***
...
// Fields returns the fields for the point.
func (p *point) Fields() (Fields, error) ***REMOVED***
	if p.cachedFields != nil ***REMOVED***
...
func (p *point) SetPrecision(precision string) ***REMOVED***
	switch precision ***REMOVED***
	case "n":
	case "u":
...
func (p *point) AppendString(buf []byte) []byte ***REMOVED***
...
// StringSize returns the length of the string that would be returned by String().
func (p *point) StringSize() int ***REMOVED***
	...
	if !p.time.IsZero() ***REMOVED***
		...
		if t < 0 ***REMOVED***
			...
			t = -t
		***REMOVED***
		for t > 9 ***REMOVED*** // already accounted for one digit
			digits++
			t /= 10
		***REMOVED***
...
// MarshalBinary returns a binary representation of the point.
func (p *point) MarshalBinary() ([]byte, error) ***REMOVED***
	if len(p.fields) == 0 ***REMOVED***
		return nil, ErrPointMustHaveAField
	***REMOVED***
...
func (p *point) UnmarshalBinary(b []byte) error ***REMOVED***
	var n int
...
// is a timestamp associated with the point, then it will be rounded to the
// given duration.
func (p *point) RoundedString(d time.Duration) string ***REMOVED***
	if p.Time().IsZero() ***REMOVED***
		return fmt.Sprintf("%s %s", p.Key(), string(p.fields))
	***REMOVED***
	return fmt.Sprintf("%s %s %d", p.Key(), string(p.fields),
...
func (p *point) unmarshalBinary() (Fields, error) ***REMOVED***
	iter := p.FieldIterator()
...
// HashID returns a non-cryptographic checksum of the point's key.
func (p *point) HashID() uint64 ***REMOVED***
	h := NewInlineFNV64a()
	h.Write(p.key)
	sum := h.Sum64()
	return sum
...
// string representations are no longer than size. Points with a single field or
// a point without a timestamp may exceed the requested size.
func (p *point) Split(size int) []Point ***REMOVED***
	if p.time.IsZero() || p.StringSize() <= size ***REMOVED***
		return []Point***REMOVED***p***REMOVED***
	***REMOVED***
	...
	for cur < len(p.fields) ***REMOVED***
		end, _ := scanTo(p.fields, cur, '=')
		end, _ = scanFieldValue(p.fields, end+1)

		if cur > start && end-start > size ***REMOVED***
			points = append(points, &point***REMOVED***
				key:    p.key,
				time:   p.time,
...
func NewTag(key, value []byte) Tag ***REMOVED***
	return Tag***REMOVED***
		Key:   key,
		Value: value,
	***REMOVED***
...
func (t Tag) Size() int ***REMOVED*** return len(t.Key) + len(t.Value) ***REMOVED***
...
func NewTags(m map[string]string) Tags ***REMOVED***
	if len(m) == 0 ***REMOVED***
		return nil
	***REMOVED***
	a := make(Tags, 0, len(m))
...
func (a Tags) String() string ***REMOVED***
	var buf bytes.Buffer
	buf.WriteByte('[')
	for i := range a ***REMOVED***
...
func (a Tags) Clone() Tags ***REMOVED***
	if len(a) == 0 ***REMOVED***
		return nil
	***REMOVED***
...
func (a Tags) Len() int           ***REMOVED*** return len(a) ***REMOVED***
func (a Tags) Less(i, j int) bool ***REMOVED*** return bytes.Compare(a[i].Key, a[j].Key) == -1 ***REMOVED***
func (a Tags) Swap(i, j int)      ***REMOVED*** a[i], a[j] = a[j], a[i] ***REMOVED***

// Equal returns true if a equals other.
func (a Tags) Equal(other Tags) bool ***REMOVED***
	if len(a) != len(other) ***REMOVED***
		return false
	***REMOVED***
	for i := range a ***REMOVED***
		if !bytes.Equal(a[i].Key, other[i].Key) || !bytes.Equal(a[i].Value, other[i].Value) ***REMOVED***
			return false
		***REMOVED***
	***REMOVED***
	return true
...
func CompareTags(a, b Tags) int ***REMOVED***
	// Compare each key & value until a mismatch.
	for i := 0; i < len(a) && i < len(b); i++ ***REMOVED***
		if cmp := bytes.Compare(a[i].Key, b[i].Key); cmp != 0 ***REMOVED***
			return cmp
		***REMOVED***
		if cmp := bytes.Compare(a[i].Value, b[i].Value); cmp != 0 ***REMOVED***
			return cmp
		***REMOVED***
	***REMOVED***
	...
	if len(a) < len(b) ***REMOVED***
		return -1
	***REMOVED*** else if len(a) > len(b) ***REMOVED***
		return 1
	***REMOVED***
...
func (a Tags) Get(key []byte) []byte ***REMOVED***
...
func (a Tags) GetString(key string) string ***REMOVED***
	return string(a.Get([]byte(key)))
***REMOVED***

// Set sets the value for a key.
func (a *Tags) Set(key, value []byte) ***REMOVED***
	for i, t := range *a ***REMOVED***
		...
		***REMOVED***
	***REMOVED***
	*a = append(*a, Tag***REMOVED***Key: key, Value: value***REMOVED***)
	sort.Sort(*a)
***REMOVED***

// SetString sets the string value for a string key.
func (a *Tags) SetString(key, value string) ***REMOVED***
...
func CopyTags(a Tags) Tags ***REMOVED***
...
func DeepCopyTags(a Tags) Tags ***REMOVED***
...
// FieldIterator retuns a FieldIterator that can be used to traverse the
// fields of a point without constructing the in-memory map.
func (p *point) FieldIterator() FieldIterator ***REMOVED***
	p.Reset()
	return p
***REMOVED***

type fieldIterator struct ***REMOVED***
	start, end  int
	key, keybuf []byte
	valueBuf    []byte
	fieldType   FieldType
...
// Next indicates whether there any fields remaining.
func (p *point) Next() bool ***REMOVED***
	p.it.start = p.it.end
	if p.it.start >= len(p.fields) ***REMOVED***
		return false
	***REMOVED***

	p.it.end, p.it.key = scanTo(p.fields, p.it.start, '=')
	if escape.IsEscaped(p.it.key) ***REMOVED***
	...
	***REMOVED***
	...
	if len(p.it.valueBuf) == 0 ***REMOVED***
	...
	***REMOVED***
	...
	if strings.IndexByte(`0123456789-.nNiIu`, c) >= 0 ***REMOVED***
		if p.it.valueBuf[len(p.it.valueBuf)-1] == 'i' ***REMOVED***
			p.it.fieldType = Integer
			p.it.valueBuf = p.it.valueBuf[:len(p.it.valueBuf)-1]
		***REMOVED*** else if p.it.valueBuf[len(p.it.valueBuf)-1] == 'u' ***REMOVED***
...
// FieldKey returns the key of the current field.
func (p *point) FieldKey() []byte ***REMOVED***
	return p.it.key
***REMOVED***

// Type returns the FieldType of the current field.
func (p *point) Type() FieldType ***REMOVED***
	return p.it.fieldType
***REMOVED***

// StringValue returns the string value of the current field.
func (p *point) StringValue() string ***REMOVED***
	return unescapeStringField(string(p.it.valueBuf[1 : len(p.it.valueBuf)-1]))
***REMOVED***

// IntegerValue returns the integer value of the current field.
func (p *point) IntegerValue() (int64, error) ***REMOVED***
...
// UnsignedValue returns the unsigned value of the current field.
func (p *point) UnsignedValue() (uint64, error) ***REMOVED***
...
// BooleanValue returns the boolean value of the current field.
func (p *point) BooleanValue() (bool, error) ***REMOVED***
...
// FloatValue returns the float value of the current field.
func (p *point) FloatValue() (float64, error) ***REMOVED***
...
func (p Fields) MarshalBinary() []byte ***REMOVED***
	var b []byte
...
func appendField(b []byte, k string, v interface***REMOVED******REMOVED***) []byte ***REMOVED***
