...
func Key(password, salt []byte, iterCount, keyLen int, h func() hash.Hash) []byte ***REMOVED***
	prf := hmac.New(h, password)
	hLen := prf.Size()
	numBlocks := keyLen / hLen
	// Get an extra block if keyLen is not an even number of hLen blocks.
	if keyLen%hLen > 0 ***REMOVED***
		numBlocks++
	***REMOVED***
	...
	for i := uint32(1); i <= uint32(numBlocks); i++ ***REMOVED***
		...
		// Ti = U1 ^ U2 ^ ... ^ Ux
		copy(Ti, Uj)
		for j := 2; j <= iterCount; j++ ***REMOVED***
			prf.Reset()
			prf.Write(Uj)
			Uj = Uj[:0]
			Uj = prf.Sum(Uj)
			for k := range Uj ***REMOVED***
				Ti[k] ^= Uj[k]
			***REMOVED***
		***REMOVED***
