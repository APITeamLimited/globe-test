...
func newClient(username, password, authzID string, fcn HashGeneratorFcn) *Client ***REMOVED***
...
func (c *Client) WithMinIterations(n int) *Client ***REMOVED***
	c.Lock()
	defer c.Unlock()
	c.minIters = n
	return c
...
func (c *Client) WithNonceGenerator(ng NonceGeneratorFcn) *Client ***REMOVED***
	c.Lock()
	defer c.Unlock()
	c.nonceGen = ng
	return c
...
func (c *Client) NewConversation() *ClientConversation ***REMOVED***
	c.RLock()
	defer c.RUnlock()
	return &ClientConversation***REMOVED***
		client:   c,
		nonceGen: c.nonceGen,
		hashGen:  c.hashGen,
		minIters: c.minIters,
...
func (c *Client) getDerivedKeys(kf KeyFactors) derivedKeys ***REMOVED***
	dk, ok := c.getCache(kf)
	if !ok ***REMOVED***
		dk = c.computeKeys(kf)
		c.setCache(kf, dk)
...
func (c *Client) GetStoredCredentials(kf KeyFactors) StoredCredentials ***REMOVED***
	dk := c.getDerivedKeys(kf)
	return StoredCredentials***REMOVED***
		KeyFactors: kf,
...
func (c *Client) computeKeys(kf KeyFactors) derivedKeys ***REMOVED***
	h := c.hashGen()
...
func (c *Client) getCache(kf KeyFactors) (derivedKeys, bool) ***REMOVED***
	c.RLock()
	defer c.RUnlock()
	dk, ok := c.cache[kf]
	return dk, ok
...
func (c *Client) setCache(kf KeyFactors, dk derivedKeys) ***REMOVED***
	c.Lock()
	defer c.Unlock()
	c.cache[kf] = dk
	return
