...
func (api API) Handler() http.Handler ***REMOVED***
	return api.router.Handler()
***REMOVED***

//Router returns the specified router on an api instance
func (api API) Router() routing.Routeable ***REMOVED***
	return api.router
***REMOVED***

// SetContextAllocator custom implementation for making contexts
func (api *API) SetContextAllocator(allocator APIContextAllocatorFunc) ***REMOVED***
...
// `resource` should be either an empty struct instance such as `Post***REMOVED******REMOVED***` or a pointer to
// a struct such as `&Post***REMOVED******REMOVED***`. The same type will be used for constructing new elements.
func (api *API) AddResource(prototype jsonapi.MarshalIdentifier, source interface***REMOVED******REMOVED***) ***REMOVED***
...
func (api *API) UseMiddleware(middleware ...HandlerFunc) ***REMOVED***
...
func (api *API) NewAPIVersion(prefix string) *API ***REMOVED***
...
func NewAPIWithResolver(prefix string, resolver URLResolver) *API ***REMOVED***
	handler := notAllowedHandler***REMOVED******REMOVED***
...
func NewAPIWithBaseURL(prefix string, baseURL string) *API ***REMOVED***
	handler := notAllowedHandler***REMOVED******REMOVED***
...
func NewAPI(prefix string) *API ***REMOVED***
	handler := notAllowedHandler***REMOVED******REMOVED***
...
func NewAPIWithRouting(prefix string, resolver URLResolver, router routing.Routeable) *API ***REMOVED***
	return newAPI(prefix, resolver, router)
***REMOVED***

// newAPI is now an internal method that can be changed if params are changing
func newAPI(prefix string, resolver URLResolver, router routing.Routeable) *API ***REMOVED***
	// Add initial and trailing slash to prefix
	prefixSlashes := strings.Trim(prefix, "/")
	if len(prefixSlashes) > 0 ***REMOVED***
		prefixSlashes = "/" + prefixSlashes + "/"
	***REMOVED*** else ***REMOVED***
		prefixSlashes = "/"
	***REMOVED***

	info := information***REMOVED***prefix: prefix, resolver: resolver***REMOVED***
	...
	***REMOVED***
	...
	api.contextPool.New = func() interface***REMOVED******REMOVED*** ***REMOVED***
		if api.contextAllocator != nil ***REMOVED***
