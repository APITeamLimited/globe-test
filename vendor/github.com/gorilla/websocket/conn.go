...
func newConn(conn net.Conn, isServer bool, readBufferSize, writeBufferSize int) *Conn ***REMOVED***
...
func newConnBRW(conn net.Conn, isServer bool, readBufferSize, writeBufferSize int, brw *bufio.ReadWriter) *Conn ***REMOVED***
	mu := make(chan bool, 1)
	mu <- true

	var br *bufio.Reader
	if readBufferSize == 0 && brw != nil && brw.Reader != nil ***REMOVED***
		// Reuse the supplied bufio.Reader if the buffer has a useful size.
		// This code assumes that peek on a reader returns
		// bufio.Reader.buf[:0].
		brw.Reader.Reset(conn)
		if p, err := brw.Reader.Peek(0); err == nil && cap(p) >= 256 ***REMOVED***
			br = brw.Reader
		***REMOVED***
	***REMOVED***
	if br == nil ***REMOVED***
		...
		***REMOVED***
		if readBufferSize < maxControlFramePayloadSize ***REMOVED***
	...
	***REMOVED***
	...
	var writeBuf []byte
	if writeBufferSize == 0 && brw != nil && brw.Writer != nil ***REMOVED***
		...
		brw.Flush()
		if cap(wh.p) >= maxFrameHeaderSize+256 ***REMOVED***
			writeBuf = wh.p[:cap(wh.p)]
		***REMOVED***
	***REMOVED***

	if writeBuf == nil ***REMOVED***
...
func (c *Conn) write(frameType int, deadline time.Time, bufs ...[]byte) error ***REMOVED***
	<-c.mu
	defer func() ***REMOVED*** c.mu <- true ***REMOVED***()
...
func (c *Conn) prepWrite(messageType int) error ***REMOVED***
...
func (c *Conn) NextWriter(messageType int) (io.WriteCloser, error) ***REMOVED***
	if err := c.prepWrite(messageType); err != nil ***REMOVED***
		return nil, err
	***REMOVED***

	mw := &messageWriter***REMOVED***
		c:         c,
...
func (w *messageWriter) fatal(err error) error ***REMOVED***
...
func (w *messageWriter) Close() error ***REMOVED***
	if w.err != nil ***REMOVED***
		return w.err
	***REMOVED***
	if err := w.flushFrame(true, nil); err != nil ***REMOVED***
		return err
	***REMOVED***
...
func (c *Conn) WriteMessage(messageType int, data []byte) error ***REMOVED***
	...
	if c.isServer && (c.newCompressionWriter == nil || !c.enableWriteCompression) ***REMOVED***
		...
		if err := c.prepWrite(messageType); err != nil ***REMOVED***
			return err
		***REMOVED***
		mw := messageWriter***REMOVED***c: c, frameType: messageType, pos: maxFrameHeaderSize***REMOVED***
...
func (c *Conn) SetCloseHandler(h func(code int, text string) error) ***REMOVED***
	if h == nil ***REMOVED***
		h = func(code int, text string) error ***REMOVED***
			...
			if code != CloseNoStatusReceived ***REMOVED***
