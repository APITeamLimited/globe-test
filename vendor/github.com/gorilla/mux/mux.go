...
func (r *Router) Match(req *http.Request, match *RouteMatch) bool ***REMOVED***
	...
	***REMOVED***
	...
	if match.MatchErr == ErrMethodMismatch && r.MethodNotAllowedHandler != nil ***REMOVED***
...
func getPath(req *http.Request) string ***REMOVED***
	if req.RequestURI != "" ***REMOVED***
		...
		if i := strings.LastIndex(path, "?"); i > -1 ***REMOVED***
			path = path[:i]
		***REMOVED***
		if i := strings.LastIndex(path, "#"); i > -1 ***REMOVED***
			path = path[:i]
