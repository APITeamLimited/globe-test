...
if isMatch, _ := re.MatchString(`Something to match`); isMatch ***REMOVED***
    //do something
***REMOVED***
```
...
```go
if m, _ := re.FindStringMatch(`Something to match`); m != nil ***REMOVED***
...
```go
func regexp2FindAllString(re *regexp2.Regexp, s string) []string ***REMOVED***
	var matches []string
...
Additionally a Unicode mode is provided which allows parsing of `\u***REMOVED***CodePoint***REMOVED***` syntax that is only when both are provided.
