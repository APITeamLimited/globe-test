...
func (a digestAlgorithm) String() string ***REMOVED***
	switch a ***REMOVED***
	case MD5:
		return "MD5"
	case SHA256:
...
func Check(req *http.Request, username, password string) bool ***REMOVED***
	auth := parseAuthorizationHeader(req.Header.Get("Authorization"))
	if auth == nil || auth.username != username ***REMOVED***
		return false
	***REMOVED***
...
func Challenge(realm string, algorithm digestAlgorithm) string ***REMOVED***
...
func sanitizeRealm(realm string) string ***REMOVED***
...
type authorization struct ***REMOVED***
...
func parseAuthorizationHeader(value string) *authorization ***REMOVED***
	if value == "" ***REMOVED***
		return nil
	***REMOVED***

	parts := strings.SplitN(value, " ", 2)
	if parts[0] != "Digest" || len(parts) != 2 ***REMOVED***
		return nil
	***REMOVED***
	...
	algo := MD5
	if strings.ToLower(auth["algorithm"]) == "sha-256" ***REMOVED***
		algo = SHA256
	***REMOVED***
...
func parseDictHeader(value string) map[string]string ***REMOVED***
...
func hash(data []byte, algorithm digestAlgorithm) string ***REMOVED***
...
func makeHA1(realm, username, password string, algorithm digestAlgorithm) string ***REMOVED***
...
//
// and H is one of MD5 or SHA256.
func makeHA2(auth *authorization, method, uri string) string ***REMOVED***
...
func response(auth *authorization, password, method, uri string) string ***REMOVED***
	ha1 := makeHA1(auth.realm, auth.username, password, auth.algorithm)
	ha2 := makeHA2(auth, method, uri)

	var r string
	if auth.qop == "auth" || auth.qop == "auth-int" ***REMOVED***
...
func compare(x, y string) bool ***REMOVED***
