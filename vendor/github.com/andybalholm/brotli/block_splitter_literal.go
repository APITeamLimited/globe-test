...
func initialEntropyCodesLiteral(data []byte, length uint, stride uint, num_histograms uint, histograms []histogramLiteral) ***REMOVED***
	var seed uint32 = 7
...
func randomSampleLiteral(seed *uint32, data []byte, length uint, stride uint, sample *histogramLiteral) ***REMOVED***
	var pos uint = 0
...
func refineEntropyCodesLiteral(data []byte, length uint, stride uint, num_histograms uint, histograms []histogramLiteral) ***REMOVED***
...
func findBlocksLiteral(data []byte, length uint, block_switch_bitcost float64, num_histograms uint, histograms []histogramLiteral, insert_cost []float64, cost []float64, switch_signal []byte, block_id []byte) uint ***REMOVED***
...
func remapBlockIdsLiteral(block_ids []byte, length uint, new_id []uint16, num_histograms uint) uint ***REMOVED***
	...
	***REMOVED***
	...
	for i = 0; i < length; i++ ***REMOVED***
		...
		if new_id[block_ids[i]] == remapBlockIdsLiteral_kInvalidId ***REMOVED***
...
func buildBlockHistogramsLiteral(data []byte, length uint, block_ids []byte, num_histograms uint, histograms []histogramLiteral) ***REMOVED***
	var i uint
	clearHistogramsLiteral(histograms, num_histograms)
	for i = 0; i < length; i++ ***REMOVED***
...
func clusterBlocksLiteral(data []byte, length uint, num_blocks uint, block_ids []byte, split *blockSplit) ***REMOVED***
	...
	***REMOVED***
		...
		for i = 0; i < num_blocks; i++ ***REMOVED***
			...
			***REMOVED***
			...
			if new_index[best_out] == clusterBlocksLiteral_kInvalidIndex ***REMOVED***
...
func splitByteVectorLiteral(data []byte, length uint, literals_per_histogram uint, max_histograms uint, sampling_stride_length uint, block_switch_cost float64, params *encoderParams, split *blockSplit) ***REMOVED***
	var data_size uint = histogramDataSizeLiteral()
