...
func encodeWindowBits(lgwin int, large_window bool, bw *bitWriter) ***REMOVED***
...
func writeMetaBlockInternal(data []byte, mask uint, last_flush_pos uint64, bytes uint, is_last bool, literal_context_mode int, params *encoderParams, prev_byte byte, prev_byte2 byte, num_literals uint, commands []command, saved_dist_cache []int, dist_cache []int, bw *bitWriter) ***REMOVED***
	...
	***REMOVED***
	...
	if bytes+4 < bw.getPos()>>3 ***REMOVED***
...
func writeMetadataHeader(s *Writer, block_size uint) ***REMOVED***
	bw := &s.bw
...
func encoderCompressStreamFast(s *Writer, op int, available_in *uint, next_in *[]byte) bool ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		if s.stream_state_ == streamFlushRequested && s.bw.nbits&7 != 0 ***REMOVED***
