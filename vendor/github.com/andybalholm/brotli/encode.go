...
func getBrotliStorage(s *Writer, size uint) []byte ***REMOVED***
	if s.storage_size_ < size ***REMOVED***
		s.storage_ = nil
...
var initCommandPrefixCodes_kDefaultCommandDepths = [128]byte***REMOVED***
	0,
	4,
	4,
	5,
	...
	12,
***REMOVED***
var initCommandPrefixCodes_kDefaultCommandBits = [128]uint16***REMOVED***
	0,
	0,
	8,
	9,
	...
	4095,
***REMOVED***
var initCommandPrefixCodes_kDefaultCommandCode = []byte***REMOVED***
	0xff,
	0x77,
	0xd5,
	0xbf,
...
func initCommandPrefixCodes(cmd_depths []byte, cmd_bits []uint16, cmd_code []byte, cmd_code_numbits *uint) ***REMOVED***
...
func writeMetaBlockInternal(data []byte, mask uint, last_flush_pos uint64, bytes uint, is_last bool, literal_context_mode int, params *encoderParams, prev_byte byte, prev_byte2 byte, num_literals uint, num_commands uint, commands []command, saved_dist_cache []int, dist_cache []int, storage_ix *uint, storage []byte) ***REMOVED***
	...
	***REMOVED***
	...
	if !shouldCompress_encode(data, mask, last_flush_pos, bytes, num_literals, num_commands) ***REMOVED***
...
/*
   ...
   input_block_size().
*/
func encodeData(s *Writer, is_last bool, force_flush bool, out_size *uint, output *[]byte) bool ***REMOVED***
	var delta uint64 = unprocessedInputSize(s)
	...
	***REMOVED***
	...
	if s.params.quality == fastTwoPassCompressionQuality && s.command_buf_ == nil ***REMOVED***
	...
	***REMOVED***
		...
		if newsize > s.cmd_alloc_size_ ***REMOVED***
	...
	***REMOVED***
	...
	if s.num_commands_ != 0 && s.last_insert_len_ == 0 ***REMOVED***
	...
	***REMOVED***
		...
		if !is_last && !force_flush && !should_flush && next_input_fits_metablock && s.num_literals_ < max_literals && s.num_commands_ < max_commands ***REMOVED***
...
func checkFlushComplete(s *Writer) ***REMOVED***
	if s.stream_state_ == streamFlushRequested && s.available_out_ == 0 ***REMOVED***
		s.stream_state_ = streamProcessing
		s.next_out_ = nil
	***REMOVED***
***REMOVED***
...
func encoderCompressStreamFast(s *Writer, op int, available_in *uint, next_in *[]byte) bool ***REMOVED***
	...
	***REMOVED***
	...
	if s.params.quality == fastTwoPassCompressionQuality ***REMOVED***
		if s.command_buf_ == nil && buf_size == kCompressFragmentTwoPassBlockSize ***REMOVED***
			s.command_buf_ = make([]uint32, kCompressFragmentTwoPassBlockSize)
			s.literal_buf_ = make([]byte, kCompressFragmentTwoPassBlockSize)
		***REMOVED***

		if s.command_buf_ != nil ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if s.available_out_ == 0 && s.stream_state_ == streamProcessing && (*available_in != 0 || op != int(operationProcess)) ***REMOVED***
	...
	***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		...
		if s.available_out_ == 0 && s.stream_state_ == streamProcessing ***REMOVED***
...
func encoderHasMoreOutput(s *Writer) bool ***REMOVED***
	return s.available_out_ != 0
***REMOVED***

func encoderTakeOutput(s *Writer) []byte ***REMOVED***
	if s.available_out_ == 0 ***REMOVED***
		return nil
	***REMOVED***
