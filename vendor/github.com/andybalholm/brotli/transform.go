...
type transforms struct ***REMOVED***
...
func transformPrefixId(t *transforms, I int) byte ***REMOVED***
	return t.transforms[(I*3)+0]
***REMOVED***

func transformType(t *transforms, I int) byte ***REMOVED***
	return t.transforms[(I*3)+1]
***REMOVED***

func transformSuffixId(t *transforms, I int) byte ***REMOVED***
	return t.transforms[(I*3)+2]
***REMOVED***

func transformPrefix(t *transforms, I int) []byte ***REMOVED***
	return t.prefix_suffix[t.prefix_suffix_map[transformPrefixId(t, I)]:]
***REMOVED***

func transformSuffix(t *transforms, I int) []byte ***REMOVED***
...
var kPrefixSuffixMap = [50]uint16***REMOVED***
	0x00,
	0x02,
	0x05,
	0x0E,
...
/* RFC 7932 transforms */
var kTransformsData = []byte***REMOVED***
	49,
	transformIdentity,
	49,
	49,
...
var kBrotliTransforms = transforms***REMOVED***
	217,
	[]byte(kPrefixSuffix),
	kPrefixSuffixMap[:],
	121,
	...
	[transformsMaxCutOff + 1]int16***REMOVED***0, 12, 27, 23, 42, 63, 56, 48, 59, 64***REMOVED***,
***REMOVED***

func getTransforms() *transforms ***REMOVED***
	return &kBrotliTransforms
***REMOVED***

func toUpperCase(p []byte) int ***REMOVED***
	if p[0] < 0xC0 ***REMOVED***
		if p[0] >= 'a' && p[0] <= 'z' ***REMOVED***
			p[0] ^= 32
		***REMOVED***
...
func shiftTransform(word []byte, word_len int, parameter uint16) int ***REMOVED***
	/* Limited sign extension: scalar < (1 << 24). */
	var scalar uint32 = (uint32(parameter) & 0x7FFF) + (0x1000000 - (uint32(parameter) & 0x8000))
	if word[0] < 0x80 ***REMOVED***
		/* 1-byte rune / 0sssssss / 7 bit scalar (ASCII). */
		scalar += uint32(word[0])

		word[0] = byte(scalar & 0x7F)
		return 1
	***REMOVED*** else if word[0] < 0xC0 ***REMOVED***
		/* Continuation / 10AAAAAA. */
		return 1
	***REMOVED*** else if word[0] < 0xE0 ***REMOVED***
		/* 2-byte rune / 110sssss AAssssss / 11 bit scalar. */
		if word_len < 2 ***REMOVED***
			return 1
		***REMOVED***
		...
		return 2
	***REMOVED*** else if word[0] < 0xF0 ***REMOVED***
		/* 3-byte rune / 1110ssss AAssssss BBssssss / 16 bit scalar. */
		if word_len < 3 ***REMOVED***
			return word_len
		***REMOVED***
		...
		return 3
	***REMOVED*** else if word[0] < 0xF8 ***REMOVED***
		/* 4-byte rune / 11110sss AAssssss BBssssss CCssssss / 21 bit scalar. */
		if word_len < 4 ***REMOVED***
			return word_len
		***REMOVED***
...
func transformDictionaryWord(dst []byte, word []byte, len int, trans *transforms, transform_idx int) int ***REMOVED***
	var idx int = 0
	...
	***REMOVED***
		var t int = int(type_)
		var i int = 0
		if t <= transformOmitLast9 ***REMOVED***
			len -= t
		***REMOVED*** else if t >= transformOmitFirst1 && t <= transformOmitFirst9 ***REMOVED***
		...
		***REMOVED***
		...
		for i < len ***REMOVED***
			...
			i++
		***REMOVED***
		if t == transformUppercaseFirst ***REMOVED***
			toUpperCase(dst[idx-len:])
		***REMOVED*** else if t == transformUppercaseAll ***REMOVED***
			...
			for len > 0 ***REMOVED***
				...
				len -= step
			***REMOVED***
		***REMOVED*** else if t == transformShiftFirst ***REMOVED***
		...
		***REMOVED*** else if t == transformShiftAll ***REMOVED***
			var param uint16 = uint16(trans.params[transform_idx*2]) + uint16(trans.params[transform_idx*2+1])<<8
			var shift []byte = dst
			shift = shift[idx-len:]
			for len > 0 ***REMOVED***
