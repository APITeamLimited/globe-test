...
func computeDistanceCode(distance uint, max_distance uint, dist_cache []int) uint ***REMOVED***
	if distance <= max_distance ***REMOVED***
		...
		if distance == uint(dist_cache[0]) ***REMOVED***
			return 0
		***REMOVED*** else if distance == uint(dist_cache[1]) ***REMOVED***
			return 1
		***REMOVED*** else if offset0 < 7 ***REMOVED***
			return (0x9750468 >> (4 * offset0)) & 0xF
		***REMOVED*** else if offset1 < 7 ***REMOVED***
			return (0xFDB1ACE >> (4 * offset1)) & 0xF
		***REMOVED*** else if distance == uint(dist_cache[2]) ***REMOVED***
			return 2
		***REMOVED*** else if distance == uint(dist_cache[3]) ***REMOVED***
			return 3
		***REMOVED***
	***REMOVED***
...
func createBackwardReferences(num_bytes uint, position uint, ringbuffer []byte, ringbuffer_mask uint, params *encoderParams, hasher hasherHandle, dist_cache []int, last_insert_len *uint, commands *[]command, num_literals *uint) ***REMOVED***
	...
	***REMOVED***
	...
	if sr2 == nil ***REMOVED***
		sr2 = &hasherSearchResult***REMOVED******REMOVED***
	***REMOVED***
	sr, _ := hasherSearchResultPool.Get().(*hasherSearchResult)
	if sr == nil ***REMOVED***
		sr = &hasherSearchResult***REMOVED******REMOVED***
	***REMOVED***

	for position+hasher.HashTypeLength() < pos_end ***REMOVED***
		...
		if sr.score > kMinScore ***REMOVED***
			/* Found a match. Let's look for something even better ahead. */
			var delayed_backward_references_in_row int = 0
			max_length--
			for ; ; max_length-- ***REMOVED***
				var cost_diff_lazy uint = 175
				if params.quality < minQualityForExtensiveReferenceSearch ***REMOVED***
				...
				***REMOVED***
				...
				if sr2.score >= sr.score+cost_diff_lazy ***REMOVED***
					...
					*sr = *sr2
					...
					if delayed_backward_references_in_row < 4 && position+hasher.HashTypeLength() < pos_end ***REMOVED***
						continue
					***REMOVED***
				***REMOVED***
			...
			***REMOVED***
				...
				if (sr.distance <= (max_distance + gap)) && distance_code > 0 ***REMOVED***
			...
			***REMOVED***
				...
				if sr.distance < sr.len>>2 ***REMOVED***
		...
		***REMOVED*** else ***REMOVED***
			...
			/* If we have not seen matches for a long time, we can skip some
			   ...
			   a lot. */
			if position > apply_random_heuristics ***REMOVED***
				/* Going through uncompressible data, jump. */
				if position > apply_random_heuristics+4*random_heuristics_window_size ***REMOVED***
					...
					for ; position < pos_jump; position += 4 ***REMOVED***
				...
				***REMOVED*** else ***REMOVED***
					...
					for ; position < pos_jump; position += 2 ***REMOVED***
						hasher.Store(ringbuffer, ringbuffer_mask, position)
						insert_length += 2
					***REMOVED***
				***REMOVED***
