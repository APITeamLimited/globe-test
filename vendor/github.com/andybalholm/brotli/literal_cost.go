...
func utf8Position(last uint, c uint, clamp uint) uint ***REMOVED***
	if c < 128 ***REMOVED***
		return 0 /* Next one is the 'Byte 1' again. */
	***REMOVED*** else if c >= 192 ***REMOVED*** /* Next one is the 'Byte 2' of utf-8 encoding. */
		return brotli_min_size_t(1, clamp)
	***REMOVED*** else ***REMOVED***
		/* Let's decide over the last byte if this ends the sequence. */
		if last < 0xE0 ***REMOVED***
...
func decideMultiByteStatsLevel(pos uint, len uint, mask uint, data []byte) uint ***REMOVED***
	var counts = [3]uint***REMOVED***0***REMOVED*** /* should be 2, but 1 compresses better. */
	var max_utf8 uint = 1
	var last_c uint = 0
	var i uint
	...
	***REMOVED***
	...
	if counts[2] < 500 ***REMOVED***
		max_utf8 = 1
	***REMOVED***

	if counts[1]+counts[2] < 25 ***REMOVED***
		max_utf8 = 0
	***REMOVED***
...
func estimateBitCostsForLiteralsUTF8(pos uint, len uint, mask uint, data []byte, cost []float32) ***REMOVED***
	var max_utf8 uint = decideMultiByteStatsLevel(pos, uint(len), mask, data)
	/* Bootstrap histograms. */
	var histogram = [3][256]uint***REMOVED***[256]uint***REMOVED***0***REMOVED******REMOVED***
	var window_half uint = 495
	var in_window uint = brotli_min_size_t(window_half, uint(len))
	var in_window_utf8 = [3]uint***REMOVED***0***REMOVED***
	...
	***REMOVED***
	...
	for i = 0; i < len; i++ ***REMOVED***
		if i >= window_half ***REMOVED***
			var c uint
			var last_c uint
			if i < window_half+1 ***REMOVED***
				c = 0
			***REMOVED*** else ***REMOVED***
				c = uint(data[(pos+i-window_half-1)&mask])
			***REMOVED***
			if i < window_half+2 ***REMOVED***
				last_c = 0
			***REMOVED*** else ***REMOVED***
...
func estimateBitCostsForLiterals(pos uint, len uint, mask uint, data []byte, cost []float32) ***REMOVED***
	if isMostlyUTF8(data, pos, mask, uint(len), kMinUTF8Ratio) ***REMOVED***
		estimateBitCostsForLiteralsUTF8(pos, uint(len), mask, data, cost)
		return
	***REMOVED*** else ***REMOVED***
		var histogram = [256]uint***REMOVED***0***REMOVED***
