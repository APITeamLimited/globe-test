...
func findMatchLengthWithLimit(s1 []byte, s2 []byte, limit uint) uint ***REMOVED***
	var matched uint = 0
	_, _ = s1[limit-1], s2[limit-1] // bounds check
	switch runtime.GOARCH ***REMOVED***
	case "amd64":
		// Compare 8 bytes at at time.
		for matched+8 <= limit ***REMOVED***
	...
	case "386":
		...
		***REMOVED***
	***REMOVED***
	for matched < limit && s1[matched] == s2[matched] ***REMOVED***
		matched++
	***REMOVED***
	return matched
***REMOVED***
