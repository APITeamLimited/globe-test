// Code generated by protoc-gen-go. DO NOT EDIT.
// source: grpc/binlog/v1/binarylog.proto

package grpc_binarylog_v1

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	duration "github.com/golang/protobuf/ptypes/duration"
	timestamp "github.com/golang/protobuf/ptypes/timestamp"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

// Enumerates the type of event
// Note the terminology is different from the RPC semantics
// definition, but the same meaning is expressed here.
type GrpcLogEntry_EventType int32

const (
	GrpcLogEntry_EVENT_TYPE_UNKNOWN GrpcLogEntry_EventType = 0
	// Header sent from client to server
	GrpcLogEntry_EVENT_TYPE_CLIENT_HEADER GrpcLogEntry_EventType = 1
	// Header sent from server to client
	GrpcLogEntry_EVENT_TYPE_SERVER_HEADER GrpcLogEntry_EventType = 2
	// Message sent from client to server
	GrpcLogEntry_EVENT_TYPE_CLIENT_MESSAGE GrpcLogEntry_EventType = 3
	// Message sent from server to client
	GrpcLogEntry_EVENT_TYPE_SERVER_MESSAGE GrpcLogEntry_EventType = 4
	// A signal that client is done sending
	GrpcLogEntry_EVENT_TYPE_CLIENT_HALF_CLOSE GrpcLogEntry_EventType = 5
	// Trailer indicates the end of the RPC.
	// On client side, this event means a trailer was either received
	// from the network or the gRPC library locally generated a status
	// to inform the application about a failure.
	// On server side, this event means the server application requested
	// to send a trailer. Note: EVENT_TYPE_CANCEL may still arrive after
	// this due to races on server side.
	GrpcLogEntry_EVENT_TYPE_SERVER_TRAILER GrpcLogEntry_EventType = 6
	// A signal that the RPC is cancelled. On client side, this
	// indicates the client application requests a cancellation.
	// On server side, this indicates that cancellation was detected.
	// Note: This marks the end of the RPC. Events may arrive after
	// this due to races. For example, on client side a trailer
	// may arrive even though the application requested to cancel the RPC.
	GrpcLogEntry_EVENT_TYPE_CANCEL GrpcLogEntry_EventType = 7
)

var GrpcLogEntry_EventType_name = map[int32]string***REMOVED***
	0: "EVENT_TYPE_UNKNOWN",
	1: "EVENT_TYPE_CLIENT_HEADER",
	2: "EVENT_TYPE_SERVER_HEADER",
	3: "EVENT_TYPE_CLIENT_MESSAGE",
	4: "EVENT_TYPE_SERVER_MESSAGE",
	5: "EVENT_TYPE_CLIENT_HALF_CLOSE",
	6: "EVENT_TYPE_SERVER_TRAILER",
	7: "EVENT_TYPE_CANCEL",
***REMOVED***

var GrpcLogEntry_EventType_value = map[string]int32***REMOVED***
	"EVENT_TYPE_UNKNOWN":           0,
	"EVENT_TYPE_CLIENT_HEADER":     1,
	"EVENT_TYPE_SERVER_HEADER":     2,
	"EVENT_TYPE_CLIENT_MESSAGE":    3,
	"EVENT_TYPE_SERVER_MESSAGE":    4,
	"EVENT_TYPE_CLIENT_HALF_CLOSE": 5,
	"EVENT_TYPE_SERVER_TRAILER":    6,
	"EVENT_TYPE_CANCEL":            7,
***REMOVED***

func (x GrpcLogEntry_EventType) String() string ***REMOVED***
	return proto.EnumName(GrpcLogEntry_EventType_name, int32(x))
***REMOVED***

func (GrpcLogEntry_EventType) EnumDescriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***0, 0***REMOVED***
***REMOVED***

// Enumerates the entity that generates the log entry
type GrpcLogEntry_Logger int32

const (
	GrpcLogEntry_LOGGER_UNKNOWN GrpcLogEntry_Logger = 0
	GrpcLogEntry_LOGGER_CLIENT  GrpcLogEntry_Logger = 1
	GrpcLogEntry_LOGGER_SERVER  GrpcLogEntry_Logger = 2
)

var GrpcLogEntry_Logger_name = map[int32]string***REMOVED***
	0: "LOGGER_UNKNOWN",
	1: "LOGGER_CLIENT",
	2: "LOGGER_SERVER",
***REMOVED***

var GrpcLogEntry_Logger_value = map[string]int32***REMOVED***
	"LOGGER_UNKNOWN": 0,
	"LOGGER_CLIENT":  1,
	"LOGGER_SERVER":  2,
***REMOVED***

func (x GrpcLogEntry_Logger) String() string ***REMOVED***
	return proto.EnumName(GrpcLogEntry_Logger_name, int32(x))
***REMOVED***

func (GrpcLogEntry_Logger) EnumDescriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***0, 1***REMOVED***
***REMOVED***

type Address_Type int32

const (
	Address_TYPE_UNKNOWN Address_Type = 0
	// address is in 1.2.3.4 form
	Address_TYPE_IPV4 Address_Type = 1
	// address is in IPv6 canonical form (RFC5952 section 4)
	// The scope is NOT included in the address string.
	Address_TYPE_IPV6 Address_Type = 2
	// address is UDS string
	Address_TYPE_UNIX Address_Type = 3
)

var Address_Type_name = map[int32]string***REMOVED***
	0: "TYPE_UNKNOWN",
	1: "TYPE_IPV4",
	2: "TYPE_IPV6",
	3: "TYPE_UNIX",
***REMOVED***

var Address_Type_value = map[string]int32***REMOVED***
	"TYPE_UNKNOWN": 0,
	"TYPE_IPV4":    1,
	"TYPE_IPV6":    2,
	"TYPE_UNIX":    3,
***REMOVED***

func (x Address_Type) String() string ***REMOVED***
	return proto.EnumName(Address_Type_name, int32(x))
***REMOVED***

func (Address_Type) EnumDescriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***7, 0***REMOVED***
***REMOVED***

// Log entry we store in binary logs
type GrpcLogEntry struct ***REMOVED***
	// The timestamp of the binary log message
	Timestamp *timestamp.Timestamp `protobuf:"bytes,1,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	// Uniquely identifies a call. The value must not be 0 in order to disambiguate
	// from an unset value.
	// Each call may have several log entries, they will all have the same call_id.
	// Nothing is guaranteed about their value other than they are unique across
	// different RPCs in the same gRPC process.
	CallId uint64 `protobuf:"varint,2,opt,name=call_id,json=callId,proto3" json:"call_id,omitempty"`
	// The entry sequence id for this call. The first GrpcLogEntry has a
	// value of 1, to disambiguate from an unset value. The purpose of
	// this field is to detect missing entries in environments where
	// durability or ordering is not guaranteed.
	SequenceIdWithinCall uint64                 `protobuf:"varint,3,opt,name=sequence_id_within_call,json=sequenceIdWithinCall,proto3" json:"sequence_id_within_call,omitempty"`
	Type                 GrpcLogEntry_EventType `protobuf:"varint,4,opt,name=type,proto3,enum=grpc.binarylog.v1.GrpcLogEntry_EventType" json:"type,omitempty"`
	Logger               GrpcLogEntry_Logger    `protobuf:"varint,5,opt,name=logger,proto3,enum=grpc.binarylog.v1.GrpcLogEntry_Logger" json:"logger,omitempty"`
	// The logger uses one of the following fields to record the payload,
	// according to the type of the log entry.
	//
	// Types that are valid to be assigned to Payload:
	//	*GrpcLogEntry_ClientHeader
	//	*GrpcLogEntry_ServerHeader
	//	*GrpcLogEntry_Message
	//	*GrpcLogEntry_Trailer
	Payload isGrpcLogEntry_Payload `protobuf_oneof:"payload"`
	// true if payload does not represent the full message or metadata.
	PayloadTruncated bool `protobuf:"varint,10,opt,name=payload_truncated,json=payloadTruncated,proto3" json:"payload_truncated,omitempty"`
	// Peer address information, will only be recorded on the first
	// incoming event. On client side, peer is logged on
	// EVENT_TYPE_SERVER_HEADER normally or EVENT_TYPE_SERVER_TRAILER in
	// the case of trailers-only. On server side, peer is always
	// logged on EVENT_TYPE_CLIENT_HEADER.
	Peer                 *Address `protobuf:"bytes,11,opt,name=peer,proto3" json:"peer,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED*** `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
***REMOVED***

func (m *GrpcLogEntry) Reset()         ***REMOVED*** *m = GrpcLogEntry***REMOVED******REMOVED*** ***REMOVED***
func (m *GrpcLogEntry) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*GrpcLogEntry) ProtoMessage()    ***REMOVED******REMOVED***
func (*GrpcLogEntry) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***0***REMOVED***
***REMOVED***

func (m *GrpcLogEntry) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_GrpcLogEntry.Unmarshal(m, b)
***REMOVED***
func (m *GrpcLogEntry) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_GrpcLogEntry.Marshal(b, m, deterministic)
***REMOVED***
func (m *GrpcLogEntry) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_GrpcLogEntry.Merge(m, src)
***REMOVED***
func (m *GrpcLogEntry) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_GrpcLogEntry.Size(m)
***REMOVED***
func (m *GrpcLogEntry) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_GrpcLogEntry.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_GrpcLogEntry proto.InternalMessageInfo

func (m *GrpcLogEntry) GetTimestamp() *timestamp.Timestamp ***REMOVED***
	if m != nil ***REMOVED***
		return m.Timestamp
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetCallId() uint64 ***REMOVED***
	if m != nil ***REMOVED***
		return m.CallId
	***REMOVED***
	return 0
***REMOVED***

func (m *GrpcLogEntry) GetSequenceIdWithinCall() uint64 ***REMOVED***
	if m != nil ***REMOVED***
		return m.SequenceIdWithinCall
	***REMOVED***
	return 0
***REMOVED***

func (m *GrpcLogEntry) GetType() GrpcLogEntry_EventType ***REMOVED***
	if m != nil ***REMOVED***
		return m.Type
	***REMOVED***
	return GrpcLogEntry_EVENT_TYPE_UNKNOWN
***REMOVED***

func (m *GrpcLogEntry) GetLogger() GrpcLogEntry_Logger ***REMOVED***
	if m != nil ***REMOVED***
		return m.Logger
	***REMOVED***
	return GrpcLogEntry_LOGGER_UNKNOWN
***REMOVED***

type isGrpcLogEntry_Payload interface ***REMOVED***
	isGrpcLogEntry_Payload()
***REMOVED***

type GrpcLogEntry_ClientHeader struct ***REMOVED***
	ClientHeader *ClientHeader `protobuf:"bytes,6,opt,name=client_header,json=clientHeader,proto3,oneof"`
***REMOVED***

type GrpcLogEntry_ServerHeader struct ***REMOVED***
	ServerHeader *ServerHeader `protobuf:"bytes,7,opt,name=server_header,json=serverHeader,proto3,oneof"`
***REMOVED***

type GrpcLogEntry_Message struct ***REMOVED***
	Message *Message `protobuf:"bytes,8,opt,name=message,proto3,oneof"`
***REMOVED***

type GrpcLogEntry_Trailer struct ***REMOVED***
	Trailer *Trailer `protobuf:"bytes,9,opt,name=trailer,proto3,oneof"`
***REMOVED***

func (*GrpcLogEntry_ClientHeader) isGrpcLogEntry_Payload() ***REMOVED******REMOVED***

func (*GrpcLogEntry_ServerHeader) isGrpcLogEntry_Payload() ***REMOVED******REMOVED***

func (*GrpcLogEntry_Message) isGrpcLogEntry_Payload() ***REMOVED******REMOVED***

func (*GrpcLogEntry_Trailer) isGrpcLogEntry_Payload() ***REMOVED******REMOVED***

func (m *GrpcLogEntry) GetPayload() isGrpcLogEntry_Payload ***REMOVED***
	if m != nil ***REMOVED***
		return m.Payload
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetClientHeader() *ClientHeader ***REMOVED***
	if x, ok := m.GetPayload().(*GrpcLogEntry_ClientHeader); ok ***REMOVED***
		return x.ClientHeader
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetServerHeader() *ServerHeader ***REMOVED***
	if x, ok := m.GetPayload().(*GrpcLogEntry_ServerHeader); ok ***REMOVED***
		return x.ServerHeader
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetMessage() *Message ***REMOVED***
	if x, ok := m.GetPayload().(*GrpcLogEntry_Message); ok ***REMOVED***
		return x.Message
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetTrailer() *Trailer ***REMOVED***
	if x, ok := m.GetPayload().(*GrpcLogEntry_Trailer); ok ***REMOVED***
		return x.Trailer
	***REMOVED***
	return nil
***REMOVED***

func (m *GrpcLogEntry) GetPayloadTruncated() bool ***REMOVED***
	if m != nil ***REMOVED***
		return m.PayloadTruncated
	***REMOVED***
	return false
***REMOVED***

func (m *GrpcLogEntry) GetPeer() *Address ***REMOVED***
	if m != nil ***REMOVED***
		return m.Peer
	***REMOVED***
	return nil
***REMOVED***

// XXX_OneofWrappers is for the internal use of the proto package.
func (*GrpcLogEntry) XXX_OneofWrappers() []interface***REMOVED******REMOVED*** ***REMOVED***
	return []interface***REMOVED******REMOVED******REMOVED***
		(*GrpcLogEntry_ClientHeader)(nil),
		(*GrpcLogEntry_ServerHeader)(nil),
		(*GrpcLogEntry_Message)(nil),
		(*GrpcLogEntry_Trailer)(nil),
	***REMOVED***
***REMOVED***

type ClientHeader struct ***REMOVED***
	// This contains only the metadata from the application.
	Metadata *Metadata `protobuf:"bytes,1,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// The name of the RPC method, which looks something like:
	// /<service>/<method>
	// Note the leading "/" character.
	MethodName string `protobuf:"bytes,2,opt,name=method_name,json=methodName,proto3" json:"method_name,omitempty"`
	// A single process may be used to run multiple virtual
	// servers with different identities.
	// The authority is the name of such a server identitiy.
	// It is typically a portion of the URI in the form of
	// <host> or <host>:<port> .
	Authority string `protobuf:"bytes,3,opt,name=authority,proto3" json:"authority,omitempty"`
	// the RPC timeout
	Timeout              *duration.Duration `protobuf:"bytes,4,opt,name=timeout,proto3" json:"timeout,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED***           `json:"-"`
	XXX_unrecognized     []byte             `json:"-"`
	XXX_sizecache        int32              `json:"-"`
***REMOVED***

func (m *ClientHeader) Reset()         ***REMOVED*** *m = ClientHeader***REMOVED******REMOVED*** ***REMOVED***
func (m *ClientHeader) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*ClientHeader) ProtoMessage()    ***REMOVED******REMOVED***
func (*ClientHeader) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***1***REMOVED***
***REMOVED***

func (m *ClientHeader) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_ClientHeader.Unmarshal(m, b)
***REMOVED***
func (m *ClientHeader) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_ClientHeader.Marshal(b, m, deterministic)
***REMOVED***
func (m *ClientHeader) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_ClientHeader.Merge(m, src)
***REMOVED***
func (m *ClientHeader) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_ClientHeader.Size(m)
***REMOVED***
func (m *ClientHeader) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_ClientHeader.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_ClientHeader proto.InternalMessageInfo

func (m *ClientHeader) GetMetadata() *Metadata ***REMOVED***
	if m != nil ***REMOVED***
		return m.Metadata
	***REMOVED***
	return nil
***REMOVED***

func (m *ClientHeader) GetMethodName() string ***REMOVED***
	if m != nil ***REMOVED***
		return m.MethodName
	***REMOVED***
	return ""
***REMOVED***

func (m *ClientHeader) GetAuthority() string ***REMOVED***
	if m != nil ***REMOVED***
		return m.Authority
	***REMOVED***
	return ""
***REMOVED***

func (m *ClientHeader) GetTimeout() *duration.Duration ***REMOVED***
	if m != nil ***REMOVED***
		return m.Timeout
	***REMOVED***
	return nil
***REMOVED***

type ServerHeader struct ***REMOVED***
	// This contains only the metadata from the application.
	Metadata             *Metadata `protobuf:"bytes,1,opt,name=metadata,proto3" json:"metadata,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED***  `json:"-"`
	XXX_unrecognized     []byte    `json:"-"`
	XXX_sizecache        int32     `json:"-"`
***REMOVED***

func (m *ServerHeader) Reset()         ***REMOVED*** *m = ServerHeader***REMOVED******REMOVED*** ***REMOVED***
func (m *ServerHeader) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*ServerHeader) ProtoMessage()    ***REMOVED******REMOVED***
func (*ServerHeader) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***2***REMOVED***
***REMOVED***

func (m *ServerHeader) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_ServerHeader.Unmarshal(m, b)
***REMOVED***
func (m *ServerHeader) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_ServerHeader.Marshal(b, m, deterministic)
***REMOVED***
func (m *ServerHeader) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_ServerHeader.Merge(m, src)
***REMOVED***
func (m *ServerHeader) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_ServerHeader.Size(m)
***REMOVED***
func (m *ServerHeader) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_ServerHeader.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_ServerHeader proto.InternalMessageInfo

func (m *ServerHeader) GetMetadata() *Metadata ***REMOVED***
	if m != nil ***REMOVED***
		return m.Metadata
	***REMOVED***
	return nil
***REMOVED***

type Trailer struct ***REMOVED***
	// This contains only the metadata from the application.
	Metadata *Metadata `protobuf:"bytes,1,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// The gRPC status code.
	StatusCode uint32 `protobuf:"varint,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	// An original status message before any transport specific
	// encoding.
	StatusMessage string `protobuf:"bytes,3,opt,name=status_message,json=statusMessage,proto3" json:"status_message,omitempty"`
	// The value of the 'grpc-status-details-bin' metadata key. If
	// present, this is always an encoded 'google.rpc.Status' message.
	StatusDetails        []byte   `protobuf:"bytes,4,opt,name=status_details,json=statusDetails,proto3" json:"status_details,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED*** `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
***REMOVED***

func (m *Trailer) Reset()         ***REMOVED*** *m = Trailer***REMOVED******REMOVED*** ***REMOVED***
func (m *Trailer) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*Trailer) ProtoMessage()    ***REMOVED******REMOVED***
func (*Trailer) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***3***REMOVED***
***REMOVED***

func (m *Trailer) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_Trailer.Unmarshal(m, b)
***REMOVED***
func (m *Trailer) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_Trailer.Marshal(b, m, deterministic)
***REMOVED***
func (m *Trailer) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_Trailer.Merge(m, src)
***REMOVED***
func (m *Trailer) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_Trailer.Size(m)
***REMOVED***
func (m *Trailer) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_Trailer.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_Trailer proto.InternalMessageInfo

func (m *Trailer) GetMetadata() *Metadata ***REMOVED***
	if m != nil ***REMOVED***
		return m.Metadata
	***REMOVED***
	return nil
***REMOVED***

func (m *Trailer) GetStatusCode() uint32 ***REMOVED***
	if m != nil ***REMOVED***
		return m.StatusCode
	***REMOVED***
	return 0
***REMOVED***

func (m *Trailer) GetStatusMessage() string ***REMOVED***
	if m != nil ***REMOVED***
		return m.StatusMessage
	***REMOVED***
	return ""
***REMOVED***

func (m *Trailer) GetStatusDetails() []byte ***REMOVED***
	if m != nil ***REMOVED***
		return m.StatusDetails
	***REMOVED***
	return nil
***REMOVED***

// Message payload, used by CLIENT_MESSAGE and SERVER_MESSAGE
type Message struct ***REMOVED***
	// Length of the message. It may not be the same as the length of the
	// data field, as the logging payload can be truncated or omitted.
	Length uint32 `protobuf:"varint,1,opt,name=length,proto3" json:"length,omitempty"`
	// May be truncated or omitted.
	Data                 []byte   `protobuf:"bytes,2,opt,name=data,proto3" json:"data,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED*** `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
***REMOVED***

func (m *Message) Reset()         ***REMOVED*** *m = Message***REMOVED******REMOVED*** ***REMOVED***
func (m *Message) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*Message) ProtoMessage()    ***REMOVED******REMOVED***
func (*Message) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***4***REMOVED***
***REMOVED***

func (m *Message) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_Message.Unmarshal(m, b)
***REMOVED***
func (m *Message) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_Message.Marshal(b, m, deterministic)
***REMOVED***
func (m *Message) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_Message.Merge(m, src)
***REMOVED***
func (m *Message) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_Message.Size(m)
***REMOVED***
func (m *Message) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_Message.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_Message proto.InternalMessageInfo

func (m *Message) GetLength() uint32 ***REMOVED***
	if m != nil ***REMOVED***
		return m.Length
	***REMOVED***
	return 0
***REMOVED***

func (m *Message) GetData() []byte ***REMOVED***
	if m != nil ***REMOVED***
		return m.Data
	***REMOVED***
	return nil
***REMOVED***

// A list of metadata pairs, used in the payload of client header,
// server header, and server trailer.
// Implementations may omit some entries to honor the header limits
// of GRPC_BINARY_LOG_CONFIG.
//
// Header keys added by gRPC are omitted. To be more specific,
// implementations will not log the following entries, and this is
// not to be treated as a truncation:
// - entries handled by grpc that are not user visible, such as those
//   that begin with 'grpc-' (with exception of grpc-trace-bin)
//   or keys like 'lb-token'
// - transport specific entries, including but not limited to:
//   ':path', ':authority', 'content-encoding', 'user-agent', 'te', etc
// - entries added for call credentials
//
// Implementations must always log grpc-trace-bin if it is present.
// Practically speaking it will only be visible on server side because
// grpc-trace-bin is managed by low level client side mechanisms
// inaccessible from the application level. On server side, the
// header is just a normal metadata key.
// The pair will not count towards the size limit.
type Metadata struct ***REMOVED***
	Entry                []*MetadataEntry `protobuf:"bytes,1,rep,name=entry,proto3" json:"entry,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED***         `json:"-"`
	XXX_unrecognized     []byte           `json:"-"`
	XXX_sizecache        int32            `json:"-"`
***REMOVED***

func (m *Metadata) Reset()         ***REMOVED*** *m = Metadata***REMOVED******REMOVED*** ***REMOVED***
func (m *Metadata) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*Metadata) ProtoMessage()    ***REMOVED******REMOVED***
func (*Metadata) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***5***REMOVED***
***REMOVED***

func (m *Metadata) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_Metadata.Unmarshal(m, b)
***REMOVED***
func (m *Metadata) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_Metadata.Marshal(b, m, deterministic)
***REMOVED***
func (m *Metadata) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_Metadata.Merge(m, src)
***REMOVED***
func (m *Metadata) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_Metadata.Size(m)
***REMOVED***
func (m *Metadata) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_Metadata.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_Metadata proto.InternalMessageInfo

func (m *Metadata) GetEntry() []*MetadataEntry ***REMOVED***
	if m != nil ***REMOVED***
		return m.Entry
	***REMOVED***
	return nil
***REMOVED***

// A metadata key value pair
type MetadataEntry struct ***REMOVED***
	Key                  string   `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	Value                []byte   `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED*** `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
***REMOVED***

func (m *MetadataEntry) Reset()         ***REMOVED*** *m = MetadataEntry***REMOVED******REMOVED*** ***REMOVED***
func (m *MetadataEntry) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*MetadataEntry) ProtoMessage()    ***REMOVED******REMOVED***
func (*MetadataEntry) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***6***REMOVED***
***REMOVED***

func (m *MetadataEntry) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_MetadataEntry.Unmarshal(m, b)
***REMOVED***
func (m *MetadataEntry) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_MetadataEntry.Marshal(b, m, deterministic)
***REMOVED***
func (m *MetadataEntry) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_MetadataEntry.Merge(m, src)
***REMOVED***
func (m *MetadataEntry) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_MetadataEntry.Size(m)
***REMOVED***
func (m *MetadataEntry) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_MetadataEntry.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_MetadataEntry proto.InternalMessageInfo

func (m *MetadataEntry) GetKey() string ***REMOVED***
	if m != nil ***REMOVED***
		return m.Key
	***REMOVED***
	return ""
***REMOVED***

func (m *MetadataEntry) GetValue() []byte ***REMOVED***
	if m != nil ***REMOVED***
		return m.Value
	***REMOVED***
	return nil
***REMOVED***

// Address information
type Address struct ***REMOVED***
	Type    Address_Type `protobuf:"varint,1,opt,name=type,proto3,enum=grpc.binarylog.v1.Address_Type" json:"type,omitempty"`
	Address string       `protobuf:"bytes,2,opt,name=address,proto3" json:"address,omitempty"`
	// only for TYPE_IPV4 and TYPE_IPV6
	IpPort               uint32   `protobuf:"varint,3,opt,name=ip_port,json=ipPort,proto3" json:"ip_port,omitempty"`
	XXX_NoUnkeyedLiteral struct***REMOVED******REMOVED*** `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
***REMOVED***

func (m *Address) Reset()         ***REMOVED*** *m = Address***REMOVED******REMOVED*** ***REMOVED***
func (m *Address) String() string ***REMOVED*** return proto.CompactTextString(m) ***REMOVED***
func (*Address) ProtoMessage()    ***REMOVED******REMOVED***
func (*Address) Descriptor() ([]byte, []int) ***REMOVED***
	return fileDescriptor_b7972e58de45083a, []int***REMOVED***7***REMOVED***
***REMOVED***

func (m *Address) XXX_Unmarshal(b []byte) error ***REMOVED***
	return xxx_messageInfo_Address.Unmarshal(m, b)
***REMOVED***
func (m *Address) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) ***REMOVED***
	return xxx_messageInfo_Address.Marshal(b, m, deterministic)
***REMOVED***
func (m *Address) XXX_Merge(src proto.Message) ***REMOVED***
	xxx_messageInfo_Address.Merge(m, src)
***REMOVED***
func (m *Address) XXX_Size() int ***REMOVED***
	return xxx_messageInfo_Address.Size(m)
***REMOVED***
func (m *Address) XXX_DiscardUnknown() ***REMOVED***
	xxx_messageInfo_Address.DiscardUnknown(m)
***REMOVED***

var xxx_messageInfo_Address proto.InternalMessageInfo

func (m *Address) GetType() Address_Type ***REMOVED***
	if m != nil ***REMOVED***
		return m.Type
	***REMOVED***
	return Address_TYPE_UNKNOWN
***REMOVED***

func (m *Address) GetAddress() string ***REMOVED***
	if m != nil ***REMOVED***
		return m.Address
	***REMOVED***
	return ""
***REMOVED***

func (m *Address) GetIpPort() uint32 ***REMOVED***
	if m != nil ***REMOVED***
		return m.IpPort
	***REMOVED***
	return 0
***REMOVED***

func init() ***REMOVED***
	proto.RegisterEnum("grpc.binarylog.v1.GrpcLogEntry_EventType", GrpcLogEntry_EventType_name, GrpcLogEntry_EventType_value)
	proto.RegisterEnum("grpc.binarylog.v1.GrpcLogEntry_Logger", GrpcLogEntry_Logger_name, GrpcLogEntry_Logger_value)
	proto.RegisterEnum("grpc.binarylog.v1.Address_Type", Address_Type_name, Address_Type_value)
	proto.RegisterType((*GrpcLogEntry)(nil), "grpc.binarylog.v1.GrpcLogEntry")
	proto.RegisterType((*ClientHeader)(nil), "grpc.binarylog.v1.ClientHeader")
	proto.RegisterType((*ServerHeader)(nil), "grpc.binarylog.v1.ServerHeader")
	proto.RegisterType((*Trailer)(nil), "grpc.binarylog.v1.Trailer")
	proto.RegisterType((*Message)(nil), "grpc.binarylog.v1.Message")
	proto.RegisterType((*Metadata)(nil), "grpc.binarylog.v1.Metadata")
	proto.RegisterType((*MetadataEntry)(nil), "grpc.binarylog.v1.MetadataEntry")
	proto.RegisterType((*Address)(nil), "grpc.binarylog.v1.Address")
***REMOVED***

func init() ***REMOVED*** proto.RegisterFile("grpc/binlog/v1/binarylog.proto", fileDescriptor_b7972e58de45083a) ***REMOVED***

var fileDescriptor_b7972e58de45083a = []byte***REMOVED***
	// 904 bytes of a gzipped FileDescriptorProto
	,
***REMOVED***
