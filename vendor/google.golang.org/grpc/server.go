...
func (s *Server) addConn(st transport.ServerTransport) bool ***REMOVED***
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Server) removeConn(st transport.ServerTransport) ***REMOVED***
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.conns != nil ***REMOVED***
		delete(s.conns, st)
	...
	***REMOVED***
	...
	if pos == -1 ***REMOVED***
		...
		***REMOVED***
		...
		if err := t.WriteStatus(stream, status.New(codes.ResourceExhausted, errDesc)); err != nil ***REMOVED***
			if trInfo != nil ***REMOVED***
				trInfo.tr.LazyLog(&fmtStringer***REMOVED***"%v", []interface***REMOVED******REMOVED******REMOVED***err***REMOVED******REMOVED***, true)
				trInfo.tr.SetError()
			***REMOVED***
