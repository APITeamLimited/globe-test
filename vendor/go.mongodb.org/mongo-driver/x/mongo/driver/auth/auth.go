...
func CreateAuthenticator(name string, cred *Cred) (Authenticator, error) ***REMOVED***
	if f, ok := authFactories[name]; ok ***REMOVED***
		return f(cred)
	***REMOVED***
...
func RegisterAuthenticatorFactory(name string, factory AuthenticatorFactory) ***REMOVED***
...
type HandshakeOptions struct ***REMOVED***
	AppName               string
...
type authHandshaker struct ***REMOVED***
...
// connection.
func (ah *authHandshaker) GetHandshakeInformation(ctx context.Context, addr address.Address, conn driver.Connection) (driver.HandshakeInformation, error) ***REMOVED***
	if ah.wrapped != nil ***REMOVED***
	...
	***REMOVED***
	...
	if ah.options.Authenticator != nil ***REMOVED***
		if speculativeAuth, ok := ah.options.Authenticator.(SpeculativeAuthenticator); ok ***REMOVED***
			var err error
			ah.conversation, err = speculativeAuth.CreateSpeculativeConversation()
			if err != nil ***REMOVED***
				return driver.HandshakeInformation***REMOVED******REMOVED***, newAuthError("failed to create conversation", err)
			***REMOVED***

			firstMsg, err := ah.conversation.FirstMessage()
			if err != nil ***REMOVED***
				return driver.HandshakeInformation***REMOVED******REMOVED***, newAuthError("failed to create speculative authentication message", err)
			***REMOVED***
	...
	***REMOVED***
	...
	if err != nil ***REMOVED***
		return driver.HandshakeInformation***REMOVED******REMOVED***, newAuthError("handshake failure", err)
	***REMOVED***
	return ah.handshakeInfo, nil
***REMOVED***

// FinishHandshake performs authentication for conn if necessary.
func (ah *authHandshaker) FinishHandshake(ctx context.Context, conn driver.Connection) error ***REMOVED***
	performAuth := ah.options.PerformAuthentication
	if performAuth == nil ***REMOVED***
		performAuth = func(serv description.Server) bool ***REMOVED***
	...
	***REMOVED***
	...
	if performAuth(desc) && ah.options.Authenticator != nil ***REMOVED***
		cfg := &Config***REMOVED***
			Description:   desc,
			Connection:    conn,
			ClusterClock:  ah.options.ClusterClock,
		...
		***REMOVED***
		...
		if err := ah.authenticate(ctx, cfg); err != nil ***REMOVED***
...
func (ah *authHandshaker) authenticate(ctx context.Context, cfg *Config) error ***REMOVED***
	...
	if speculativeResponse := ah.handshakeInfo.SpeculativeAuthenticate; speculativeResponse != nil ***REMOVED***
		// Defensively ensure that the server did not include a response if speculative auth was not attempted.
		if ah.conversation == nil ***REMOVED***
...
func Handshaker(h driver.Handshaker, options *HandshakeOptions) driver.Handshaker ***REMOVED***
	return &authHandshaker***REMOVED***
		wrapped: h,
		options: options,
	***REMOVED***
***REMOVED***
...
type Authenticator interface ***REMOVED***
	// Auth authenticates the connection.
	Auth(context.Context, *Config) error
***REMOVED***

func newAuthError(msg string, inner error) error ***REMOVED***
...
func newError(err error, mech string) error ***REMOVED***
	return &Error***REMOVED***
...
func (e *Error) Inner() error ***REMOVED***
	return e.inner
***REMOVED***
...
func (e *Error) Message() string ***REMOVED***
	return e.message
***REMOVED***
