// Copyright (C) MongoDB, Inc. 2019-present.
//
// Licensed under the Apache License, Version 2.0 (the "License"); you may
// not use this file except in compliance with the License. You may obtain
// a copy of the License at http://www.apache.org/licenses/LICENSE-2.0

package operation

import (
	"context"
	"errors"
	"time"

	"go.mongodb.org/mongo-driver/bson/bsontype"
	"go.mongodb.org/mongo-driver/event"
	"go.mongodb.org/mongo-driver/mongo/description"
	"go.mongodb.org/mongo-driver/mongo/readconcern"
	"go.mongodb.org/mongo-driver/mongo/readpref"
	"go.mongodb.org/mongo-driver/x/bsonx/bsoncore"
	"go.mongodb.org/mongo-driver/x/mongo/driver"
	"go.mongodb.org/mongo-driver/x/mongo/driver/session"
)

// Find performs a find operation.
type Find struct ***REMOVED***
	allowDiskUse        *bool
	allowPartialResults *bool
	awaitData           *bool
	batchSize           *int32
	collation           bsoncore.Document
	comment             *string
	filter              bsoncore.Document
	hint                bsoncore.Value
	let                 bsoncore.Document
	limit               *int64
	max                 bsoncore.Document
	maxTimeMS           *int64
	min                 bsoncore.Document
	noCursorTimeout     *bool
	oplogReplay         *bool
	projection          bsoncore.Document
	returnKey           *bool
	showRecordID        *bool
	singleBatch         *bool
	skip                *int64
	snapshot            *bool
	sort                bsoncore.Document
	tailable            *bool
	session             *session.Client
	clock               *session.ClusterClock
	collection          string
	monitor             *event.CommandMonitor
	crypt               driver.Crypt
	database            string
	deployment          driver.Deployment
	readConcern         *readconcern.ReadConcern
	readPreference      *readpref.ReadPref
	selector            description.ServerSelector
	retry               *driver.RetryMode
	result              driver.CursorResponse
	serverAPI           *driver.ServerAPIOptions
	timeout             *time.Duration
***REMOVED***

// NewFind constructs and returns a new Find.
func NewFind(filter bsoncore.Document) *Find ***REMOVED***
	return &Find***REMOVED***
		filter: filter,
	***REMOVED***
***REMOVED***

// Result returns the result of executing this operation.
func (f *Find) Result(opts driver.CursorOptions) (*driver.BatchCursor, error) ***REMOVED***
	opts.ServerAPI = f.serverAPI
	return driver.NewBatchCursor(f.result, f.session, f.clock, opts)
***REMOVED***

func (f *Find) processResponse(info driver.ResponseInfo) error ***REMOVED***
	var err error
	f.result, err = driver.NewCursorResponse(info)
	return err
***REMOVED***

// Execute runs this operations and returns an error if the operation did not execute successfully.
func (f *Find) Execute(ctx context.Context) error ***REMOVED***
	if f.deployment == nil ***REMOVED***
		return errors.New("the Find operation must have a Deployment set before Execute can be called")
	***REMOVED***

	return driver.Operation***REMOVED***
		CommandFn:         f.command,
		ProcessResponseFn: f.processResponse,
		RetryMode:         f.retry,
		Type:              driver.Read,
		Client:            f.session,
		Clock:             f.clock,
		CommandMonitor:    f.monitor,
		Crypt:             f.crypt,
		Database:          f.database,
		Deployment:        f.deployment,
		ReadConcern:       f.readConcern,
		ReadPreference:    f.readPreference,
		Selector:          f.selector,
		Legacy:            driver.LegacyFind,
		ServerAPI:         f.serverAPI,
		Timeout:           f.timeout,
	***REMOVED***.Execute(ctx, nil)

***REMOVED***

func (f *Find) command(dst []byte, desc description.SelectedServer) ([]byte, error) ***REMOVED***
	dst = bsoncore.AppendStringElement(dst, "find", f.collection)
	if f.allowDiskUse != nil ***REMOVED***
		if desc.WireVersion == nil || !desc.WireVersion.Includes(4) ***REMOVED***
			return nil, errors.New("the 'allowDiskUse' command parameter requires a minimum server wire version of 4")
		***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "allowDiskUse", *f.allowDiskUse)
	***REMOVED***
	if f.allowPartialResults != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "allowPartialResults", *f.allowPartialResults)
	***REMOVED***
	if f.awaitData != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "awaitData", *f.awaitData)
	***REMOVED***
	if f.batchSize != nil ***REMOVED***
		dst = bsoncore.AppendInt32Element(dst, "batchSize", *f.batchSize)
	***REMOVED***
	if f.collation != nil ***REMOVED***
		if desc.WireVersion == nil || !desc.WireVersion.Includes(5) ***REMOVED***
			return nil, errors.New("the 'collation' command parameter requires a minimum server wire version of 5")
		***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "collation", f.collation)
	***REMOVED***
	if f.comment != nil ***REMOVED***
		dst = bsoncore.AppendStringElement(dst, "comment", *f.comment)
	***REMOVED***
	if f.filter != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "filter", f.filter)
	***REMOVED***
	if f.hint.Type != bsontype.Type(0) ***REMOVED***
		dst = bsoncore.AppendValueElement(dst, "hint", f.hint)
	***REMOVED***
	if f.let != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "let", f.let)
	***REMOVED***
	if f.limit != nil ***REMOVED***
		dst = bsoncore.AppendInt64Element(dst, "limit", *f.limit)
	***REMOVED***
	if f.max != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "max", f.max)
	***REMOVED***
	// Only append specified maxTimeMS if timeout is not also specified.
	if f.maxTimeMS != nil && f.timeout == nil ***REMOVED***
		dst = bsoncore.AppendInt64Element(dst, "maxTimeMS", *f.maxTimeMS)
	***REMOVED***
	if f.min != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "min", f.min)
	***REMOVED***
	if f.noCursorTimeout != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "noCursorTimeout", *f.noCursorTimeout)
	***REMOVED***
	if f.oplogReplay != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "oplogReplay", *f.oplogReplay)
	***REMOVED***
	if f.projection != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "projection", f.projection)
	***REMOVED***
	if f.returnKey != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "returnKey", *f.returnKey)
	***REMOVED***
	if f.showRecordID != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "showRecordId", *f.showRecordID)
	***REMOVED***
	if f.singleBatch != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "singleBatch", *f.singleBatch)
	***REMOVED***
	if f.skip != nil ***REMOVED***
		dst = bsoncore.AppendInt64Element(dst, "skip", *f.skip)
	***REMOVED***
	if f.snapshot != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "snapshot", *f.snapshot)
	***REMOVED***
	if f.sort != nil ***REMOVED***
		dst = bsoncore.AppendDocumentElement(dst, "sort", f.sort)
	***REMOVED***
	if f.tailable != nil ***REMOVED***
		dst = bsoncore.AppendBooleanElement(dst, "tailable", *f.tailable)
	***REMOVED***
	return dst, nil
***REMOVED***

// AllowDiskUse when true allows temporary data to be written to disk during the find command."
func (f *Find) AllowDiskUse(allowDiskUse bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.allowDiskUse = &allowDiskUse
	return f
***REMOVED***

// AllowPartialResults when true allows partial results to be returned if some shards are down.
func (f *Find) AllowPartialResults(allowPartialResults bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.allowPartialResults = &allowPartialResults
	return f
***REMOVED***

// AwaitData when true makes a cursor block before returning when no data is available.
func (f *Find) AwaitData(awaitData bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.awaitData = &awaitData
	return f
***REMOVED***

// BatchSize specifies the number of documents to return in every batch.
func (f *Find) BatchSize(batchSize int32) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.batchSize = &batchSize
	return f
***REMOVED***

// Collation specifies a collation to be used.
func (f *Find) Collation(collation bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.collation = collation
	return f
***REMOVED***

// Comment sets a string to help trace an operation.
func (f *Find) Comment(comment string) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.comment = &comment
	return f
***REMOVED***

// Filter determines what results are returned from find.
func (f *Find) Filter(filter bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.filter = filter
	return f
***REMOVED***

// Hint specifies the index to use.
func (f *Find) Hint(hint bsoncore.Value) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.hint = hint
	return f
***REMOVED***

// Let specifies the let document to use. This option is only valid for server versions 5.0 and above.
func (f *Find) Let(let bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.let = let
	return f
***REMOVED***

// Limit sets a limit on the number of documents to return.
func (f *Find) Limit(limit int64) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.limit = &limit
	return f
***REMOVED***

// Max sets an exclusive upper bound for a specific index.
func (f *Find) Max(max bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.max = max
	return f
***REMOVED***

// MaxTimeMS specifies the maximum amount of time to allow the query to run.
func (f *Find) MaxTimeMS(maxTimeMS int64) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.maxTimeMS = &maxTimeMS
	return f
***REMOVED***

// Min sets an inclusive lower bound for a specific index.
func (f *Find) Min(min bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.min = min
	return f
***REMOVED***

// NoCursorTimeout when true prevents cursor from timing out after an inactivity period.
func (f *Find) NoCursorTimeout(noCursorTimeout bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.noCursorTimeout = &noCursorTimeout
	return f
***REMOVED***

// OplogReplay when true replays a replica set's oplog.
func (f *Find) OplogReplay(oplogReplay bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.oplogReplay = &oplogReplay
	return f
***REMOVED***

// Projection limits the fields returned for all documents.
func (f *Find) Projection(projection bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.projection = projection
	return f
***REMOVED***

// ReturnKey when true returns index keys for all result documents.
func (f *Find) ReturnKey(returnKey bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.returnKey = &returnKey
	return f
***REMOVED***

// ShowRecordID when true adds a $recordId field with the record identifier to returned documents.
func (f *Find) ShowRecordID(showRecordID bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.showRecordID = &showRecordID
	return f
***REMOVED***

// SingleBatch specifies whether the results should be returned in a single batch.
func (f *Find) SingleBatch(singleBatch bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.singleBatch = &singleBatch
	return f
***REMOVED***

// Skip specifies the number of documents to skip before returning.
func (f *Find) Skip(skip int64) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.skip = &skip
	return f
***REMOVED***

// Snapshot prevents the cursor from returning a document more than once because of an intervening write operation.
func (f *Find) Snapshot(snapshot bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.snapshot = &snapshot
	return f
***REMOVED***

// Sort specifies the order in which to return results.
func (f *Find) Sort(sort bsoncore.Document) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.sort = sort
	return f
***REMOVED***

// Tailable keeps a cursor open and resumable after the last data has been retrieved.
func (f *Find) Tailable(tailable bool) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.tailable = &tailable
	return f
***REMOVED***

// Session sets the session for this operation.
func (f *Find) Session(session *session.Client) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.session = session
	return f
***REMOVED***

// ClusterClock sets the cluster clock for this operation.
func (f *Find) ClusterClock(clock *session.ClusterClock) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.clock = clock
	return f
***REMOVED***

// Collection sets the collection that this command will run against.
func (f *Find) Collection(collection string) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.collection = collection
	return f
***REMOVED***

// CommandMonitor sets the monitor to use for APM events.
func (f *Find) CommandMonitor(monitor *event.CommandMonitor) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.monitor = monitor
	return f
***REMOVED***

// Crypt sets the Crypt object to use for automatic encryption and decryption.
func (f *Find) Crypt(crypt driver.Crypt) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.crypt = crypt
	return f
***REMOVED***

// Database sets the database to run this operation against.
func (f *Find) Database(database string) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.database = database
	return f
***REMOVED***

// Deployment sets the deployment to use for this operation.
func (f *Find) Deployment(deployment driver.Deployment) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.deployment = deployment
	return f
***REMOVED***

// ReadConcern specifies the read concern for this operation.
func (f *Find) ReadConcern(readConcern *readconcern.ReadConcern) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.readConcern = readConcern
	return f
***REMOVED***

// ReadPreference set the read preference used with this operation.
func (f *Find) ReadPreference(readPreference *readpref.ReadPref) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.readPreference = readPreference
	return f
***REMOVED***

// ServerSelector sets the selector used to retrieve a server.
func (f *Find) ServerSelector(selector description.ServerSelector) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.selector = selector
	return f
***REMOVED***

// Retry enables retryable mode for this operation. Retries are handled automatically in driver.Operation.Execute based
// on how the operation is set.
func (f *Find) Retry(retry driver.RetryMode) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.retry = &retry
	return f
***REMOVED***

// ServerAPI sets the server API version for this operation.
func (f *Find) ServerAPI(serverAPI *driver.ServerAPIOptions) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.serverAPI = serverAPI
	return f
***REMOVED***

// Timeout sets the timeout for this operation.
func (f *Find) Timeout(timeout *time.Duration) *Find ***REMOVED***
	if f == nil ***REMOVED***
		f = new(Find)
	***REMOVED***

	f.timeout = timeout
	return f
***REMOVED***
