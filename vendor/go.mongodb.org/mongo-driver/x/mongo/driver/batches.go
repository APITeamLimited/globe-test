...
// than zero.
func (b *Batches) Valid() bool ***REMOVED*** return b != nil && b.Identifier != "" && len(b.Documents) > 0 ***REMOVED***

// ClearBatch clears the Current batch. This must be called before AdvanceBatch will advance to the
// next batch.
func (b *Batches) ClearBatch() ***REMOVED*** b.Current = b.Current[:0] ***REMOVED***
...
func (b *Batches) AdvanceBatch(maxCount, targetBatchSize, maxDocSize int) error ***REMOVED***
	if len(b.Current) > 0 ***REMOVED***
		return nil
	***REMOVED***
	...
	***REMOVED***
	...
	splitAfter := 0
	size := 0
	for i, doc := range b.Documents ***REMOVED***
		if i == maxCount ***REMOVED***
			break
		***REMOVED***
		if len(doc) > maxDocSize ***REMOVED***
			return ErrDocumentTooLarge
		***REMOVED***
		if size+len(doc) > targetBatchSize ***REMOVED***
			break
		***REMOVED***
