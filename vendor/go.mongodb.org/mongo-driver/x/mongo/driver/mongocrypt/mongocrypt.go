...
type MongoCrypt struct ***REMOVED***
...
func NewMongoCrypt(opts *options.MongoCryptOptions) (*MongoCrypt, error) ***REMOVED***
	...
	if wrapped == nil ***REMOVED***
		return nil, errors.New("could not create new mongocrypt object")
	***REMOVED***
	crypt := &MongoCrypt***REMOVED***
		wrapped: wrapped,
	***REMOVED***

	// set options in mongocrypt
	if err := crypt.setProviderOptions(opts.KmsProviders); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	if err := crypt.setLocalSchemaMap(opts.LocalSchemaMap); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	if err := crypt.setEncryptedFieldsMap(opts.EncryptedFieldsMap); err != nil ***REMOVED***
		return nil, err
	***REMOVED***

	if opts.BypassQueryAnalysis ***REMOVED***
	...
	***REMOVED***
	...
	if !opts.CryptSharedLibDisabled ***REMOVED***
		...
		if opts.CryptSharedLibOverridePath != "" ***REMOVED***
	...
	***REMOVED***
	...
	// initialize handle
	if !C.mongocrypt_init(crypt.wrapped) ***REMOVED***
...
func (m *MongoCrypt) CreateEncryptionContext(db string, cmd bsoncore.Document) (*Context, error) ***REMOVED***
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_encrypt_init(ctx.wrapped, dbStr, C.int32_t(-1), cmdBinary.wrapped); !ok ***REMOVED***
...
func (m *MongoCrypt) CreateDecryptionContext(cmd bsoncore.Document) (*Context, error) ***REMOVED***
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_decrypt_init(ctx.wrapped, cmdBinary.wrapped); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***
	return ctx, nil
***REMOVED***
...
func lookupString(doc bsoncore.Document, key string) string ***REMOVED***
	strVal, _ := doc.Lookup(key).StringValueOK()
	return strVal
***REMOVED***

func setAltName(ctx *Context, altName string) error ***REMOVED***
	// create document ***REMOVED***"keyAltName": keyAltName***REMOVED***
	...
	if ok := C.mongocrypt_ctx_setopt_key_alt_name(ctx.wrapped, keyAltBinary.wrapped); !ok ***REMOVED***
		return ctx.createErrorFromStatus()
	***REMOVED***
	return nil
***REMOVED***

func setKeyMaterial(ctx *Context, keyMaterial []byte) error ***REMOVED***
	// Create document ***REMOVED***"keyMaterial": keyMaterial***REMOVED*** using the generic binary sybtype 0x00.
	idx, doc := bsoncore.AppendDocumentStart(nil)
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_setopt_key_material(ctx.wrapped, keyMaterialBinary.wrapped); !ok ***REMOVED***
		return ctx.createErrorFromStatus()
	***REMOVED***
	return nil
***REMOVED***

func rewrapDataKey(ctx *Context, filter []byte) error ***REMOVED***
	filterBinary := newBinaryFromBytes(filter)
	defer filterBinary.close()

	if ok := C.mongocrypt_ctx_rewrap_many_datakey_init(ctx.wrapped, filterBinary.wrapped); !ok ***REMOVED***
		return ctx.createErrorFromStatus()
	***REMOVED***
	return nil
***REMOVED***
...
func (m *MongoCrypt) CreateDataKeyContext(kmsProvider string, opts *options.DataKeyOptions) (*Context, error) ***REMOVED***
	...
	***REMOVED***
	...
	// Create a masterKey document of the form ***REMOVED*** "provider": <provider string>, other options... ***REMOVED***.
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_setopt_key_encryption_key(ctx.wrapped, masterKeyBinary.wrapped); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***

	for _, altName := range opts.KeyAltNames ***REMOVED***
		if err := setAltName(ctx, altName); err != nil ***REMOVED***
			return nil, err
		***REMOVED***
	***REMOVED***

	if opts.KeyMaterial != nil ***REMOVED***
		if err := setKeyMaterial(ctx, opts.KeyMaterial); err != nil ***REMOVED***
			return nil, err
		***REMOVED***
	***REMOVED***

	if ok := C.mongocrypt_ctx_datakey_init(ctx.wrapped); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***
	return ctx, nil
***REMOVED***
...
func (m *MongoCrypt) CreateExplicitEncryptionContext(doc bsoncore.Document, opts *options.ExplicitEncryptionOptions) (*Context, error) ***REMOVED***
	...
	***REMOVED***
	...
	if opts.KeyID != nil ***REMOVED***
		keyIDBinary := newBinaryFromBytes(opts.KeyID.Data)
		defer keyIDBinary.close()

		if ok := C.mongocrypt_ctx_setopt_key_id(ctx.wrapped, keyIDBinary.wrapped); !ok ***REMOVED***
			return nil, ctx.createErrorFromStatus()
		***REMOVED***
	***REMOVED***
	if opts.KeyAltName != nil ***REMOVED***
		if err := setAltName(ctx, *opts.KeyAltName); err != nil ***REMOVED***
			return nil, err
		***REMOVED***
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_setopt_algorithm(ctx.wrapped, algoStr, -1); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***

	if opts.QueryType != "" ***REMOVED***
		queryStr := C.CString(opts.QueryType)
		defer C.free(unsafe.Pointer(queryStr))
		if ok := C.mongocrypt_ctx_setopt_query_type(ctx.wrapped, queryStr, -1); !ok ***REMOVED***
			return nil, ctx.createErrorFromStatus()
		***REMOVED***
	***REMOVED***

	if opts.ContentionFactor != nil ***REMOVED***
		if ok := C.mongocrypt_ctx_setopt_contention_factor(ctx.wrapped, C.int64_t(*opts.ContentionFactor)); !ok ***REMOVED***
			return nil, ctx.createErrorFromStatus()
		***REMOVED***
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_explicit_encrypt_init(ctx.wrapped, docBinary.wrapped); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***
...
func (m *MongoCrypt) CreateExplicitDecryptionContext(doc bsoncore.Document) (*Context, error) ***REMOVED***
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_ctx_explicit_decrypt_init(ctx.wrapped, docBinary.wrapped); !ok ***REMOVED***
		return nil, ctx.createErrorFromStatus()
	***REMOVED***
	return ctx, nil
***REMOVED***
	...
	***REMOVED***
	...
	if opts.Provider != nil ***REMOVED***
		...
		***REMOVED***
		...
		if ok := C.mongocrypt_ctx_setopt_key_encryption_key(ctx.wrapped, mongocryptBinary.wrapped); !ok ***REMOVED***
			return nil, ctx.createErrorFromStatus()
		***REMOVED***
	***REMOVED***
...
func (m *MongoCrypt) setProviderOptions(kmsProviders bsoncore.Document) error ***REMOVED***
	providersBinary := newBinaryFromBytes(kmsProviders)
	defer providersBinary.close()

	if ok := C.mongocrypt_setopt_kms_providers(m.wrapped, providersBinary.wrapped); !ok ***REMOVED***
...
func (m *MongoCrypt) setLocalSchemaMap(schemaMap map[string]bsoncore.Document) error ***REMOVED***
	if len(schemaMap) == 0 ***REMOVED***
		return nil
	***REMOVED***
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_setopt_schema_map(m.wrapped, schemaMapBinary.wrapped); !ok ***REMOVED***
		return m.createErrorFromStatus()
	***REMOVED***
	return nil
***REMOVED***
...
func (m *MongoCrypt) setEncryptedFieldsMap(encryptedfieldsMap map[string]bsoncore.Document) error ***REMOVED***
	if len(encryptedfieldsMap) == 0 ***REMOVED***
		return nil
	***REMOVED***
	...
	***REMOVED***
	...
	if ok := C.mongocrypt_setopt_encrypted_field_config_map(m.wrapped, encryptedfieldsMapBinary.wrapped); !ok ***REMOVED***
		return m.createErrorFromStatus()
	***REMOVED***
	return nil
***REMOVED***
...
func (m *MongoCrypt) createErrorFromStatus() error ***REMOVED***
