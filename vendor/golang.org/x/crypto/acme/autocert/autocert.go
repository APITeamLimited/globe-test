...
func HostWhitelist(hosts ...string) HostPolicy ***REMOVED***
	...
	for _, h := range hosts ***REMOVED***
		if h, err := idna.Lookup.ToASCII(h); err == nil ***REMOVED***
			whitelist[h] = true
		***REMOVED***
	***REMOVED***
...
func (c certKey) String() string ***REMOVED***
	if c.isToken ***REMOVED***
...
func (m *Manager) TLSConfig() *tls.Config ***REMOVED***
	return &tls.Config***REMOVED***
		GetCertificate: m.GetCertificate,
		NextProtos: []string***REMOVED***
...
func (m *Manager) GetCertificate(hello *tls.ClientHelloInfo) (*tls.Certificate, error) ***REMOVED***
	...
	***REMOVED***
	...
	if wantsTokenCert(hello) ***REMOVED***
		m.challengeMu.RLock()
		defer m.challengeMu.RUnlock()
		if cert := m.certTokens[name]; cert != nil ***REMOVED***
			return cert, nil
		***REMOVED***
		if cert, err := m.cacheGet(ctx, certKey***REMOVED***domain: name, isToken: true***REMOVED***); err == nil ***REMOVED***
			return cert, nil
		***REMOVED***
...
func wantsTokenCert(hello *tls.ClientHelloInfo) bool ***REMOVED***
	// tls-alpn-01
	if len(hello.SupportedProtos) == 1 && hello.SupportedProtos[0] == acme.ALPNProto ***REMOVED***
		return true
	***REMOVED***
	return false
***REMOVED***

func supportsECDSA(hello *tls.ClientHelloInfo) bool ***REMOVED***
	// The "signature_algorithms" extension, if present, limits the key exchange
	// algorithms allowed by the cipher suites. See RFC 5246, section 7.4.1.4.1.
	if hello.SignatureSchemes != nil ***REMOVED***
		ecdsaOK := false
	schemeLoop:
		for _, scheme := range hello.SignatureSchemes ***REMOVED***
			const tlsECDSAWithSHA1 tls.SignatureScheme = 0x0203 // constant added in Go 1.10
			switch scheme ***REMOVED***
		...
		***REMOVED***
	***REMOVED***
	if hello.SupportedCurves != nil ***REMOVED***
		ecdsaOK := false
		for _, curve := range hello.SupportedCurves ***REMOVED***
			if curve == tls.CurveP256 ***REMOVED***
				ecdsaOK = true
				break
			***REMOVED***
		***REMOVED***
		...
		***REMOVED***
	***REMOVED***
	for _, suite := range hello.CipherSuites ***REMOVED***
		switch suite ***REMOVED***
...
func (m *Manager) cert(ctx context.Context, ck certKey) (*tls.Certificate, error) ***REMOVED***
	m.stateMu.Lock()
	if s, ok := m.state[ck]; ok ***REMOVED***
		m.stateMu.Unlock()
		s.RLock()
		defer s.RUnlock()
		return s.tlscert()
...
func (m *Manager) cacheGet(ctx context.Context, ck certKey) (*tls.Certificate, error) ***REMOVED***
...
func (m *Manager) cachePut(ctx context.Context, ck certKey, tlscert *tls.Certificate) error ***REMOVED***
	if m.Cache == nil ***REMOVED***
		return nil
	***REMOVED***
...
func (m *Manager) createCert(ctx context.Context, ck certKey) (*tls.Certificate, error) ***REMOVED***
	...
	if err != nil ***REMOVED***
		...
		time.AfterFunc(createCertRetryAfter, func() ***REMOVED***
			...
			if !ok ***REMOVED***
				return
			***REMOVED***
			if _, err := validCert(ck, s.cert, s.key, m.now()); err == nil ***REMOVED***
				return
			***REMOVED***
			delete(m.state, ck)
		***REMOVED***)
...
func (m *Manager) certState(ck certKey) (*certState, error) ***REMOVED***
	m.stateMu.Lock()
	defer m.stateMu.Unlock()
	if m.state == nil ***REMOVED***
		m.state = make(map[certKey]*certState)
	***REMOVED***
	// existing state
	if state, ok := m.state[ck]; ok ***REMOVED***
		return state, nil
	***REMOVED***
...
func (m *Manager) authorizedCert(ctx context.Context, key crypto.Signer, ck certKey) (der [][]byte, leaf *x509.Certificate, err error) ***REMOVED***
	...
	***REMOVED***
	...
	// Pre-RFC legacy CA.
	case dir.OrderURL == "":
		if err := m.verify(ctx, client, ck.domain); err != nil ***REMOVED***
			return nil, nil, err
		***REMOVED***
	...
	for ***REMOVED***
		...
		***REMOVED***
		if chal == nil ***REMOVED***
			...
			for chal, err := range errs ***REMOVED***
...
// using each applicable ACME challenge type.
func (m *Manager) verifyRFC(ctx context.Context, client *acme.Client, domain string) (*acme.Order, error) ***REMOVED***
...
AuthorizeOrderLoop:
	for ***REMOVED***
		...
		***REMOVED***
		...
		for _, zurl := range o.AuthzURLs ***REMOVED***
			z, err := client.GetAuthorization(ctx, zurl)
			if err != nil ***REMOVED***
				return nil, err
			***REMOVED***
			if z.Status != acme.StatusPending ***REMOVED***
			...
			***REMOVED***
			...
			if _, err := client.Accept(ctx, chal); err != nil ***REMOVED***
				continue AuthorizeOrderLoop
			***REMOVED***
			if _, err := client.WaitAuthorization(ctx, z.URI); err != nil ***REMOVED***
				continue AuthorizeOrderLoop
			***REMOVED***
		***REMOVED***
...
func (m *Manager) supportedChallengeTypes() []string ***REMOVED***
	m.challengeMu.RLock()
	defer m.challengeMu.RUnlock()
	typ := []string***REMOVED***"tls-alpn-01"***REMOVED***
...
func (m *Manager) deactivatePendingAuthz(uri []string) ***REMOVED***
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	client, err := m.acmeClient(ctx)
	if err != nil ***REMOVED***
		return
	***REMOVED***
	for _, u := range uri ***REMOVED***
		z, err := client.GetAuthorization(ctx, u)
		if err == nil && z.Status == acme.StatusPending ***REMOVED***
...
func (m *Manager) fulfill(ctx context.Context, client *acme.Client, chal *acme.Challenge, domain string) (cleanup func(), err error) ***REMOVED***
	...
	case "tls-alpn-01":
		...
		***REMOVED***
		...
		return func() ***REMOVED*** go m.deleteCertToken(domain) ***REMOVED***, nil
	case "http-01":
...
func (m *Manager) putCertToken(ctx context.Context, name string, cert *tls.Certificate) ***REMOVED***
	...
	***REMOVED***
	...
	m.cachePut(ctx, certKey***REMOVED***domain: name, isToken: true***REMOVED***, cert)
***REMOVED***
...
func (m *Manager) deleteCertToken(name string) ***REMOVED***
	...
	if m.Cache != nil ***REMOVED***
		ck := certKey***REMOVED***domain: name, isToken: true***REMOVED***
...
func (m *Manager) renew(ck certKey, key crypto.Signer, exp time.Time) ***REMOVED***
	m.renewalMu.Lock()
	defer m.renewalMu.Unlock()
	if m.renewal[ck] != nil ***REMOVED***
	...
	***REMOVED***
	dr := &domainRenewal***REMOVED***m: m, ck: ck, key: key***REMOVED***
	m.renewal[ck] = dr
	dr.start(exp)
***REMOVED***
...
func (m *Manager) acmeClient(ctx context.Context) (*acme.Client, error) ***REMOVED***
	...
	***REMOVED***
	...
	if client == nil ***REMOVED***
		client = &acme.Client***REMOVED***DirectoryURL: DefaultACMEDirectory***REMOVED***
	***REMOVED***
	...
	***REMOVED***
	...
	if err == nil || isAccountAlreadyExist(err) ***REMOVED***
		m.client = client
		err = nil
	***REMOVED***
	return m.client, err
...
func isAccountAlreadyExist(err error) bool ***REMOVED***
	if err == acme.ErrAccountAlreadyExists ***REMOVED***
		return true
	***REMOVED***
...
func (m *Manager) now() time.Time ***REMOVED***
	if m.nowFunc != nil ***REMOVED***
		return m.nowFunc()
	***REMOVED***
	return time.Now()
***REMOVED***
...
func certRequest(key crypto.Signer, cn string, ext []pkix.Extension, san ...string) ([]byte, error) ***REMOVED***
...
func validCert(ck certKey, der [][]byte, key crypto.Signer, now time.Time) (leaf *x509.Certificate, err error) ***REMOVED***
	// parse public part(s)
	var n int
	...
	***REMOVED***
	...
	if err != nil || len(x509Cert) == 0 ***REMOVED***
	...
	***REMOVED***
	if err := leaf.VerifyHostname(ck.domain); err != nil ***REMOVED***
		return nil, err
	***REMOVED***
	...
	case *rsa.PublicKey:
		...
		***REMOVED***
		if !ck.isRSA && !ck.isToken ***REMOVED***
...
var (
	...
	testDidRemoveState = func(certKey) ***REMOVED******REMOVED***
)
