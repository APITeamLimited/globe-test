...
func copyStat(stat *Stat_t, statLE *Stat_LE_t) ***REMOVED***
...
func (d *Dirent) NameString() string ***REMOVED***
	if d == nil ***REMOVED***
		return ""
	***REMOVED***
...
func anyToSockaddr(_ int, rsa *RawSockaddrAny) (Sockaddr, error) ***REMOVED***
	...
	case AF_UNIX:
		...
		// length is not zero.
		if pp.Len != 0 && pp.Path[0] == 0 ***REMOVED***
		...
		***REMOVED***
		...
		n := 0
		for n < int(pp.Len) && pp.Path[n] != 0 ***REMOVED***
			n++
		***REMOVED***
...
func Fstat(fd int, stat *Stat_t) (err error) ***REMOVED***
...
func Ptsname(fd int) (name string, err error) ***REMOVED***
...
func u2s(cstr unsafe.Pointer) string ***REMOVED***
	str := (*[1024]uint8)(cstr)
	i := 0
	for str[i] != 0 ***REMOVED***
		i++
...
func Close(fd int) (err error) ***REMOVED***
	_, _, e1 := syscall_syscall(SYS_CLOSE, uintptr(fd), 0, 0)
	for i := 0; e1 == EAGAIN && i < 10; i++ ***REMOVED***
...
func Getrusage(who int, rusage *Rusage) (err error) ***REMOVED***
	var ruz rusage_zos
...
func Stat(path string, sta *Stat_t) (err error) ***REMOVED***
	var statLE Stat_LE_t
...
func Mkfifoat(dirfd int, path string, mode uint32) (err error) ***REMOVED***
	wd, err := Getwd()
	if err != nil ***REMOVED***
		return err
	***REMOVED***

	if err := Fchdir(dirfd); err != nil ***REMOVED***
		return err
	***REMOVED***
	defer Chdir(wd)
...
func setTimeval(sec, usec int64) Timeval ***REMOVED*** //fix
...
type nwmTriplet struct ***REMOVED***
	offset uint32
	length uint32
	number uint32
***REMOVED***

type nwmQuadruplet struct ***REMOVED***
	offset uint32
	length uint32
	number uint32
	match  uint32
...
type nwmRecHeader struct ***REMOVED***
	ident  uint32
	length uint32
	number byte
	_      [3]byte
...
type nwmTCPStatsEntry struct ***REMOVED***
	ident             uint64
	currEstab         uint32
	activeOpened      uint32
...
type nwmConnEntry struct ***REMOVED***
	ident             uint32
	local             [28]byte // union of sockaddr4 and sockaddr6
	remote            [28]byte // union of sockaddr4 and sockaddr6
...
var svcNameTable [][]byte = [][]byte***REMOVED***
...
func GetsockoptTCPInfo(fd, level, opt int) (*TCPInfo, error) ***REMOVED***
	...
	request := (*struct ***REMOVED***
		header nwmHeader
		filter nwmFilter
	***REMOVED***)(unsafe.Pointer(&responseBuffer[0]))

	EZBNMIF4 := svcLoad(&svcNameTable[svc_EZBNMIF4][0])
	if EZBNMIF4 == nil ***REMOVED***
	...
	***REMOVED***
	...
	if returnCode != 0 || request.header.outputDesc.offset == 0 ***REMOVED***
		return nil, errnoErr(EINVAL)
	***REMOVED***
	...
	if recHeader.ident != nwmRecHeaderIdentifier ***REMOVED***
		return nil, errnoErr(EINVAL)
	***REMOVED***
	...
	for i := uint32(0); i < uint32(recHeader.number); i++ ***REMOVED***
		...
		for j := uint32(0); j < sectionDesc.number; j++ ***REMOVED***
	...
	***REMOVED***
	...
	for _, ptr := range sections ***REMOVED***
		switch *ptr ***REMOVED***
		case nwmTCPStatsIdentifier:
			if tcpStats != nil ***REMOVED***
				return nil, errnoErr(EINVAL)
			***REMOVED***
	...
	***REMOVED***
	...
	if err != nil ***REMOVED***
		return nil, errnoErr(EINVAL)
	***REMOVED***
	if localSockaddr.Addr.Family == AF_INET ***REMOVED***
		...
		for i = 0; i < 4; i++ ***REMOVED***
			if localSockaddr.Addr[i] != 0 ***REMOVED***
				break
			***REMOVED***
		***REMOVED***
		if i != 4 ***REMOVED***
			...
			***REMOVED***
		***REMOVED***
		if localSockaddr.Port != 0 ***REMOVED***
			request.filter.flags |= nwmFilterLclPortMask
			localSockFilter.Port = localSockaddr.Port
		***REMOVED***
	***REMOVED*** else if localSockaddr.Addr.Family == AF_INET6 ***REMOVED***
	...
	***REMOVED***
	...
	if conn.ident != nwmTCPConnIdentifier ***REMOVED***
		return nil, errnoErr(EINVAL)
	***REMOVED***
...
func Opendir(name string) (uintptr, error) ***REMOVED***
...
func Readdir(dir uintptr) (*Dirent, error) ***REMOVED***
	var ent Dirent
	var res uintptr
...
func Closedir(dir uintptr) error ***REMOVED***
...
func Seekdir(dir uintptr, pos int) ***REMOVED***
	_, _, _ = syscall_syscall(SYS_SEEKDIR, dir, uintptr(pos), 0)
***REMOVED***

func Telldir(dir uintptr) (int, error) ***REMOVED***
...
func Flock(fd int, how int) error ***REMOVED***
...
func Mlock(b []byte) (err error) ***REMOVED***
...
func Mlock2(b []byte, flags int) (err error) ***REMOVED***
	_, _, e1 := syscall_syscall(SYS___MLOCKALL, _BPX_NONSWAP, 0, 0)
	if e1 != 0 ***REMOVED***
		err = errnoErr(e1)
	***REMOVED***
...
func Mlockall(flags int) (err error) ***REMOVED***
	_, _, e1 := syscall_syscall(SYS___MLOCKALL, _BPX_NONSWAP, 0, 0)
	if e1 != 0 ***REMOVED***
		err = errnoErr(e1)
	***REMOVED***
...
func Munlock(b []byte) (err error) ***REMOVED***
...
func Munlockall() (err error) ***REMOVED***
	_, _, e1 := syscall_syscall(SYS___MLOCKALL, _BPX_SWAP, 0, 0)
	if e1 != 0 ***REMOVED***
		err = errnoErr(e1)
	***REMOVED***
...
func ClockGettime(clockid int32, ts *Timespec) error ***REMOVED***
	...
	if ts == nil ***REMOVED***
		return EFAULT
	***REMOVED***
	if clockid == CLOCK_REALTIME || clockid == CLOCK_MONOTONIC ***REMOVED***
		var nanotime int64 = runtime.Nanotime1()
		ts.Sec = nanotime / nsec_per_sec
		ts.Nsec = nanotime % nsec_per_sec
	***REMOVED*** else if clockid == CLOCK_PROCESS_CPUTIME_ID || clockid == CLOCK_THREAD_CPUTIME_ID ***REMOVED***
		var tm Tms
		_, err := Times(&tm)
		if err != nil ***REMOVED***
			return EFAULT
...
func Statfs(path string, stat *Statfs_t) (err error) ***REMOVED***
...
func errnoErr(e Errno) error ***REMOVED***
	switch e ***REMOVED***
	case 0:
		return nil
	case EAGAIN:
...
func ErrnoName(e Errno) string ***REMOVED***
...
func Mount(source string, target string, fstype string, flags uintptr, data string) (err error) ***REMOVED***
	if needspace := 8 - len(fstype); needspace <= 0 ***REMOVED***
		fstype = fstype[:8]
	***REMOVED*** else ***REMOVED***
		fstype += "        "[:needspace]
	***REMOVED***
...
func Unmount(name string, mtm int) (err error) ***REMOVED***
	...
	***REMOVED***
	...
	b2s := func(arr []byte) string ***REMOVED***
	...
	***REMOVED***
	...
	if err != nil ***REMOVED***
		return err
	***REMOVED***
	if fsCount == 0 ***REMOVED***
		return EINVAL
	***REMOVED***
	for i := 0; i < fsCount; i++ ***REMOVED***
		if b2s(buffer.fsinfo[i].Mountpoint[:]) == name ***REMOVED***
