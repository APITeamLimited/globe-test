...
// On sparc***REMOVED***,64***REMOVED***, the kernel defines struct termios2 itself which clashes with the
...
(
	...
	awk '
		...
		$2 ~ /^ELF_.*$/ ***REMOVED***next***REMOVED***# <asm/elf.h> contains ELF_ARCH, etc.

		$2 ~ /^EXTATTR_NAMESPACE_NAMES/ ||
		$2 ~ /^EXTATTR_NAMESPACE_[A-Z]+_STRING/ ***REMOVED***next***REMOVED***
		...
		$2 ~ /^MEM/ ||
		$2 ~ /^WG/ ||
		$2 ~ /^BLK[A-Z]*(GET$|SET$|BUF$|PART$|SIZE)/ ***REMOVED***printf("\t%s = C.%s\n", $2, $2)***REMOVED***
...
echo
echo "//go:build $***REMOVED***GOARCH***REMOVED*** && $***REMOVED***GOOS***REMOVED***"
echo "// +build $***REMOVED***GOARCH***REMOVED***,$***REMOVED***GOOS***REMOVED***"
echo
...
struct tuple errors[] = ***REMOVED***
"
	for i in $errors
	do
		echo -E '	***REMOVED***'$i', "'$i'" ***REMOVED***,'
	done

	echo -E "
***REMOVED***;

struct tuple signals[] = ***REMOVED***
"
	for i in $signals
	do
		echo -E '	***REMOVED***'$i', "'$i'" ***REMOVED***,'
...
***REMOVED***
	...
	printf("\n\n// Error table\n");
	printf("var errorList = [...]struct ***REMOVED***\n");
	...
	for(i=0; i<nelem(errors); i++) ***REMOVED***
		...
		if(A <= buf[0] && buf[0] <= Z && a <= buf[1] && buf[1] <= z)
			buf[0] += a - A;
		printf("\t***REMOVED*** %d, \"%s\", \"%s\" ***REMOVED***,\n", e, errors[i].name, buf);
	***REMOVED***
	printf("***REMOVED***\n\n");

	printf("\n\n// Signal table\n");
	printf("var signalList = [...]struct ***REMOVED***\n");
