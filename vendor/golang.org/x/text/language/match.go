...
func (m *matcher) Match(want ...Tag) (t Tag, index int, c Confidence) ***REMOVED***
	...
	***REMOVED***
	if w.RegionID != tt.RegionID && w.RegionID != 0 ***REMOVED***
		if w.RegionID != 0 && tt.RegionID != 0 && tt.RegionID.Contains(w.RegionID) ***REMOVED***
			tt.RegionID = w.RegionID
			tt.RemakeString()
		***REMOVED*** else if r := w.RegionID.String(); len(r) == 2 ***REMOVED***
...
func makeHaveTag(tag language.Tag, index int) (haveTag, language.Language) ***REMOVED***
	max := tag
	if tag.LangID != 0 || tag.RegionID != 0 || tag.ScriptID != 0 ***REMOVED***
		max, _ = canonicalize(All, max)
		max, _ = max.Maximize()
		max.RemakeString()
	***REMOVED***
	return haveTag***REMOVED***tag, index, Exact, max.RegionID, max.ScriptID, altScript(max.LangID, max.ScriptID), 0***REMOVED***, max.LangID
***REMOVED***

// altScript returns an alternative script that may match the given script with
// a low confidence.  At the moment, the langMatch data allows for at most one
// script to map to another and we rely on this to keep the code simple.
func altScript(l language.Language, s language.Script) language.Script ***REMOVED***
	for _, alt := range matchScript ***REMOVED***
		// TODO: also match cases where language is not the same.
		if (language.Language(alt.wantLang) == l || language.Language(alt.haveLang) == l) &&
			language.Script(alt.haveScript) == s ***REMOVED***
...
func (h *matchHeader) addIfNew(n haveTag, exact bool) ***REMOVED***
	...
	for _, v := range h.haveTags ***REMOVED***
		if equalsRest(v.tag, n.tag) ***REMOVED***
			return
		***REMOVED***
	***REMOVED***
	...
	for i, v := range h.haveTags ***REMOVED***
		if v.maxScript == n.maxScript &&
			...
			v.tag.VariantOrPrivateUseTags() == n.tag.VariantOrPrivateUseTags() ***REMOVED***
...
func (m *matcher) header(l language.Language) *matchHeader ***REMOVED***
...
func newMatcher(supported []Tag, options []MatchOption) *matcher ***REMOVED***
	...
	***REMOVED***
	...
	update := func(want, have uint16, conf Confidence) ***REMOVED***
		if hh := m.index[language.Language(have)]; hh != nil ***REMOVED***
	...
	***REMOVED***
	...
	for i, lm := range language.AliasMap ***REMOVED***
		// If deprecated codes match and there is no fiddling with the script or
		// or region, we consider it an exact match.
		conf := Exact
		if language.AliasTypes[i] != language.Macro ***REMOVED***
			if !isExactEquivalent(language.Language(lm.From)) ***REMOVED***
				conf = High
			***REMOVED***
...
func (m *matcher) getBest(want ...Tag) (got *haveTag, orig language.Tag, c Confidence) ***REMOVED***
	best := bestMatch***REMOVED******REMOVED***
	for i, ww := range want ***REMOVED***
		w := ww.tag()
		var max language.Tag
		// Check for exact match first.
		h := m.index[w.LangID]
		if w.LangID != 0 ***REMOVED***
			if h == nil ***REMOVED***
				continue
			***REMOVED***
			...
			if w.RegionID != max.RegionID ***REMOVED***
		...
		***REMOVED*** else ***REMOVED***
			...
			if h != nil ***REMOVED***
				for i := range h.haveTags ***REMOVED***
					have := h.haveTags[i]
					if equalsRest(have.tag, w) ***REMOVED***
						return have, w, Exact
					***REMOVED***
				***REMOVED***
			***REMOVED***
			if w.ScriptID == 0 && w.RegionID == 0 ***REMOVED***
				// We skip all tags matching und for approximate matching, including
				// private tags.
				continue
			***REMOVED***
			max, _ = w.Maximize()
			if h = m.index[max.LangID]; h == nil ***REMOVED***
				continue
			***REMOVED***
		***REMOVED***
		pin := true
		for _, t := range want[i+1:] ***REMOVED***
			if w.LangID == t.lang() ***REMOVED***
				pin = false
				break
			***REMOVED***
		***REMOVED***
	...
	***REMOVED***
	if best.conf <= No ***REMOVED***
		...
		***REMOVED***
		return nil, language.Tag***REMOVED******REMOVED***, No
	***REMOVED***
	return best.have, best.want, best.conf
***REMOVED***
...
// be false.
func (m *bestMatch) update(have *haveTag, tag language.Tag, maxScript language.Script, maxRegion language.Region, pin bool) ***REMOVED***
	...
	***REMOVED***
	...
	if m.pinLanguage && tag.LangID != m.want.LangID ***REMOVED***
		return
	***REMOVED***
	// Pin the region group if we are comparing tags for the same language.
	if tag.LangID == m.want.LangID && m.sameRegionGroup ***REMOVED***
	...
	***REMOVED***
	if c == Exact && have.maxScript == maxScript ***REMOVED***
		...
		m.pinLanguage = pin
	***REMOVED***
	if equalsRest(have.tag, tag) ***REMOVED***
...
func isParadigmLocale(lang language.Language, r language.Region) bool ***REMOVED***
	for _, e := range paradigmLocales ***REMOVED***
		if language.Language(e[0]) == lang && (r == language.Region(e[1]) || r == language.Region(e[2])) ***REMOVED***
			return true
		***REMOVED***
	***REMOVED***
	return false
...
// CLDR grouping.
func regionGroupDist(a, b language.Region, script language.Script, lang language.Language) (dist uint8, same bool) ***REMOVED***
	const defaultDistance = 4

	aGroup := uint(regionToGroups[a]) << 1
	bGroup := uint(regionToGroups[b]) << 1
	for _, ri := range matchRegion ***REMOVED***
		if language.Language(ri.lang) == lang && (ri.script == 0 || language.Script(ri.script) == script) ***REMOVED***
...
func equalsRest(a, b language.Tag) bool ***REMOVED***
...
func isExactEquivalent(l language.Language) bool ***REMOVED***
...
func init() ***REMOVED***
	...
	// script or region.
	for _, lm := range language.AliasMap ***REMOVED***
		tag := language.Tag***REMOVED***LangID: language.Language(lm.From)***REMOVED***
		if tag, _ = canonicalize(All, tag); tag.ScriptID != 0 || tag.RegionID != 0 ***REMOVED***
			notEquivalent = append(notEquivalent, language.Language(lm.From))
		***REMOVED***
	***REMOVED***
	// Maximize undefined regions of paradigm locales.
	for i, v := range paradigmLocales ***REMOVED***
		t := language.Tag***REMOVED***LangID: language.Language(v[0])***REMOVED***
		max, _ := t.Maximize()
		if v[1] == 0 ***REMOVED***
