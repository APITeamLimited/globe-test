...
func (h *vuHandle) run(logger *logrus.Logger, flow <-chan int64, iterDone chan<- struct***REMOVED******REMOVED***) ***REMOVED***
	h.RLock()
	ctx := h.ctx
	h.RUnlock()
		...
		***REMOVED***
		...
		case t := <-ticker.C:
			...
			if end >= 0 && at >= end ***REMOVED***
				e.Logger.WithFields(logrus.Fields***REMOVED***"at": at, "end": end***REMOVED***).Debug("Local: Hit time limit")
				cutoff = time.Now()
				return nil
			***REMOVED***
		...
		case <-iterDone:
			...
			if end >= 0 && at >= end ***REMOVED***
				e.Logger.WithFields(logrus.Fields***REMOVED***"at": at, "end": end***REMOVED***).Debug("Local: Hit iteration limit")
				return nil
			***REMOVED***
		case <-ctx.Done():
...
func (e *Executor) SetLogger(l *logrus.Logger) ***REMOVED***
	e.Logger = l
***REMOVED***

// GetLogger returns current Executor's logger.
func (e *Executor) GetLogger() *logrus.Logger ***REMOVED***
	return e.Logger
***REMOVED***
